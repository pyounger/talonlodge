...
*/
	...
	{
		// Loading all items
		$items = $this->outlet->from('App_Model_Page')->where('is_template = 0')->orderBy('parent_id ASC, title ASC')->find();

		// Loading root item
		$root_item = new App_Model_Page();
		$root_item->level = 0;
		$root_item->title = t('backend.pages.root_menu_item');
		$root_item->remove_linked_objects();

		// Creating tree		
		$this->tree = new App_Model_PageTreeRoot($root_item);
