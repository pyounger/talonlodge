...
{block name='title'}
    {capture name='site_title'}{t}backend.twitter.reviews{/t}{/capture}
...
{block name='content_init'}
    ...
    {if $cpf_rights->has_rights($cpf_controller, 'add')}
        {capture name="common_actions"}
            {cpf_button title=t('backend.twitter.load_reviews') icon='retweet' url=cpf_link(['controller' => $cpf_controller, 'action' => 'load'])}
        {/capture}
    {/if}
