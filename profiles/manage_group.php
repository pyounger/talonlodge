...
if (isset($_REQUEST['op'])) {
    switch ($_REQUEST['op']) {
        ...
        case 5:
            ...
            break;
        case 6:
            $strMSG = " Total members should be less then 44 ";
            $class = "alert alert-danger";
            break;
        case 7:
            $strMSG = " Login Info sent ";
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Group<?php echo (($_SESSION["UType"]==1)?'s':'');?> <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Group<?php echo (($_SESSION["UType"]==1)?'s':'');?>: </b> You can manage group information here </p>
        </blockquote>
    </div>
</div>
...
//                }    
                        ...
                        <table class="table users-table table-condensed table-hover table-striped display dataTable" id="example1" >
                            <thead>
                                <tr>
                                    ...
                                    <th class="visible-sm visible-md visible-lg">Profiles</th>
    ...
    <?php
                ...
                if($_SESSION["UType"]==1){
                    $Query = "SELECT g.grp_id, g.Contact_ID, g.Pms_Booking_ID, g.GroupName, g.GroupArrivalDate, g.GroupDepartureDate, g.Booking_Status, g.grp_total_cust, c.ContactFirstName, c.ContactLastName, p.Package_Name, c.is_email, (select count(gcont_id) from group_contacts gc inner join contacts c on gc.ContactID = c.ContactID where gc.grp_id = g.grp_id and is_completed = 1) as total_profiles "
                            . "FROM groups AS g  LEFT OUTER JOIN contacts AS c ON g.Contact_ID=c.ContactID LEFT  OUTER JOIN packages AS p ON g.Pms_Package_ID=p.Pms_Package_ID WHERE g.GroupArrivalDate!='0000-00-00' AND g.GroupDepartureDate!='0000-00-00' AND g.Booking_Status='Confirmed' AND (g.GroupArrivalDate >= '".$_SESSION['from']."' AND g.GroupArrivalDate <= '".$_SESSION['to']."') AND ( g.GroupName LIKE '%".$_SESSION['group_name']."%' ) ORDER BY g.GroupArrivalDate ".$_SESSION['filtertime']." ";
                } else {
		    $Query = "SELECT gc.grp_id, gr.Contact_ID, gr.GroupName, gr.GroupArrivalDate, gr.GroupDepartureDate, gr.Pms_Package_ID, gr.Booking_Status, gr.grp_total_cust, p.Package_Name, c.ContactFirstName, c.ContactLastName, c.is_email, (select count(gcont_id) from group_contacts gc inner join contacts c on gc.ContactID = c.ContactID where gc.grp_id = g.grp_id and is_completed = 1) as total_profiles "
                            . "FROM group_contacts AS gc INNER JOIN groups AS gr ON gr.grp_id = gc.grp_id  INNER JOIN packages AS p ON p.Pms_Package_ID=gr.Pms_Package_ID LEFT OUTER JOIN contacts AS c ON c.ContactID=gr.Contact_ID WHERE gc.ContactID='".$_SESSION["contact_id"]."'";
...
*/
    ...
    if (mysql_num_rows($rs) > 0) {
        while ($row = mysql_fetch_object($rs)) {
            ...
            if ($row->grp_total_cust == $row->total_profiles) {
                $class_profiles = "scheduled";
            } else {
                $class_profiles = "missing-guest";
            }
            ...
            ?>
                                        <tr>
                                            ...
                                            <td class="visible-sm visible-md visible-lg"><span class="label <?php echo $class_profiles; ?>"><?php print($row->total_profiles);?></span></td>
