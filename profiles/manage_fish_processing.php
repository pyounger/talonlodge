...
if ((isset($_REQUEST['pk_from'])) && ($_REQUEST['pk_from'] != '')) {
    $_SESSION['pk_from'] = date("Y-m-d",strtotime($_REQUEST['pk_from']));
} else {
   $_SESSION['pk_from'] = date("Y-m-d",strtotime(date("Y-m-d")." -14 days"));
}
                            ...
                            <div class="col-mol-md-offset-2">
                                <form class="form-horizontal cascde-forms" method="post" action="<?php print($_SERVER['PHP_SELF']); ?>" name="frm" id="frm" >
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            <input type="text" class="form-control form-cascade-control datetimepicker" value="<?php echo calendarDateConver2($_SESSION['pk_from']);?>" id="pk_from" name="pk_from" style="width: 160px;" title="Please Enter Group Arrival Date" placeholder=" Date From ">
                                        </div>
                                    </div>
...
<!--                                        <th class="visible-xs visible-sm visible-md visible-lg">Package Start</th>
        ...
        <?php
        $Query = "SELECT p.* FROM packages AS p WHERE p.Arrival_Start_Date > '".$_SESSION['pk_from']."' ORDER BY YEAR(p.Arrival_Start_Date) DESC, p.Arrival_Start_Date ASC";
		//print($Query);
		//print("<br>");
        $counter = 0;
        $limit = $_SESSION['limit_of_rec'];
...
<!--                                            <tr>
                                                <td class="visible-xs visible-sm visible-md visible-lg">
                                                    ...
                                                    <?php
                                                        ...
                                                        LEFT OUTER JOIN jacket_size AS j ON cp.jacketsize_id=j.jacketsize_id
                                                        WHERE p.Pms_Package_ID = ".$row->Pms_Package_ID." AND p.Arrival_Start_Date > '".$_SESSION['pk_from']."'  AND p.Arrival_Start_Date = g.GroupArrivalDate AND p.Arrival_End_Date = g.GroupDepartureDate
                                                        GROUP BY g.grp_id 
                                                            ...
                                                            while ($row11 = mysql_fetch_object($rs11)) {
                                                                    ...
                                                                    echo '<tr>';
                                                                        echo '<th>';
                                                                            ...
                                                                            echo "<br />";
                                                                            echo "<a href=\"manage_fish_processing_groups.php?grp_id=".$row11->grp_id."\">Group Setup</a>";
                                                                            //echo $Query11;
                                                                        echo '</th>';
                                                                        $day_counter = 0;
                                                                        $day_counter1 = 0;
                                                                                    ...
                                                                                    echo '<th class="visible-xs visible-sm visible-md visible-lg">';
                                                                                        ...
                                                                                        if ($count22 > 0) {
                                                                                            while ($row22 = mysql_fetch_object($rs22)) {
                                                                                                echo '<br/>';
                                                                                                echo "<span class=\"scheduled\"><a href=".$_SERVER['PHP_SELF'] . "?show=1&add=1&pms_pak_id=".$row->Pms_Package_ID."&grp_id=".$row11->grp_id."&date=".$dt->format( "Y-m-d" )."&update=1>Update</a></span>";
                                                                                            }
                                                                                        } else {
                                                                                            echo '<br/>';
                                                                                            echo "<a href=".$_SERVER['PHP_SELF'] . "?show=1&add=1&pms_pak_id=".$row->Pms_Package_ID."&grp_id=".$row11->grp_id."&date=".$dt->format( "Y-m-d" )."&add=1>Process Fish</a>";
                                                                                        }                        
