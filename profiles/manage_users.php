...
if (isset($_REQUEST['action'])) {
    if (isset($_REQUEST['btnAdd'])) {
        ...
        $udtQuery = "UPDATE users SET user_pass ='" . dbStr(md5($_REQUEST['user_pass'])) . "', user_pasphrase ='" . dbStr($_REQUEST['user_pass']) . "', lastUpdated=NOW() WHERE cont_id=" . $_REQUEST['cont_id'];
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Contacts </h3>
    </div>
</div>
                    ...
                    <form name="frm" id="frm" method="post" class="form-horizontal" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" role="form">
                        <div class="form-group">
                            ...
                            <div class="col-lg-10 col-md-9">
                                <input type="text" class="form-control form-cascade-control input_wid70 required" placeholder="User Display Name..." value="<?php echo (($_REQUEST['cont_id']!=0)?$user_display_name:$_SESSION['user_display_name']);?>" id="user_display_name" name="user_display_name" readonly="true">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Username</label>
                            <div class="col-lg-10 col-md-9">
                                <input type="text" class="form-control form-cascade-control input_wid70 required" placeholder="Email..." value="<?php echo (($_REQUEST['cont_id']!=0)?$user_name:$_SESSION['UserName']);?>" id="user_name" name="user_name" readonly="true">
                            </div>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="ro">
                        <div class="col-mol-md-offset-2">
                            <form class="form-horizontal cascde-forms" method="post" action="<?php print($_SERVER['PHP_SELF']); ?>" name="frm" id="frm" >
                                <div class="form-group">
                                    <label class="col-lg-2 col-md-9 control-label">Username or Display Name:</label>
                                    <div class="col-lg-10 col-md-9">
                                        <input type="text" class="form-control form-cascade-control" value="<?php echo $_SESSION['search_name'];?>" id="search_name" name="search_name" style="width: 200px;"  placeholder="Username or Display Name">
                                    </div>
                                </div>
                ...
                <div class="panel-body">
                    <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                        <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                            <thead>
                                <tr>
                                    <th class="visible-xs visible-sm visible-md visible-lg">Username</th>
                            ...
                            <tbody>
                                <?php
                                    $Query = "SELECT DISTINCT u.*, c.ContactFirstName, c.ContactLastName FROM users AS u LEFT OUTER JOIN contacts AS c ON u.cont_id = c.ContactID WHERE u.user_id != '1' AND ( u.user_name LIKE '%".$_SESSION['search_name']."%' OR u.user_display_name LIKE '%".$_SESSION['search_name']."%' ) ";
                                    $counter = 0;
