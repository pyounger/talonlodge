...
if(isset($_REQUEST['submitAllRec'])){
    ...
    for ($i = 0; $i < count($_REQUEST['act_id']); $i++) {
    	    print("SAVING".$_REQUEST['act_id'][$i] );
        if($_REQUEST['act_id'][$i] != 0){
            if($_REQUEST['asch_id'][$i] != 0){
                $asch_start_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $asch_end_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_end_time'][$i]);
                
                $asch_start_date = date('Y-m-d H:i:s', strtotime($asch_start_date));
                $asch_end_date = date('Y-m-d H:i:s', strtotime($asch_end_date));
                
                mysql_query("UPDATE act_schedule SET act_id = ".$_REQUEST['act_id'][$i].", ag_id=".$_REQUEST['ag_id'][$i].", asch_start_date = '" . $asch_start_date . "', asch_end_date = '" . $asch_end_date . "', updatedDate = NOW(), av_id=".$_REQUEST['av_id'][$i].", act_boat_id=".$_REQUEST['act_boat_id'][$i]." WHERE asch_id = ".$_REQUEST['asch_id'][$i]." ") or die(mysql_error());
                header("Location: manage_advent_activity_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=2");
            } else {
                $asch_start_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $asch_end_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_end_time'][$i]);
                ...
                mysql_query("INSERT INTO act_schedule (asch_id, cont_id, grp_id, act_id, ag_id, asch_start_date, asch_end_date, createdDate, pms_pak_id,av_id, act_boat_id) VALUES (".$MaxID.", '".$_REQUEST['cont_id'][$i]."', '".$_REQUEST['grp_id'][$i]."', ".$_REQUEST['act_id'][$i].", ".$_REQUEST['ag_id'][$i].", '".$asch_start_date."', '".$asch_end_date."', NOW(), '".$_REQUEST['pms_pak_id']."', '".$_REQUEST['av_id'][$i]."', '".$_REQUEST['act_boat_id'][$i]."')") or die(mysql_error());
                header("Location: manage_advent_activity_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=1");
            }
        }
    }
...
if(isset($_REQUEST['dell'])){
    ...
    header("Location: manage_advent_activity_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=3");
}
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Activities Assignment <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b> Massage Activities Assignment </b> You can Manage Adventure Activity Assignment here </p>
        </blockquote>
    </div>
</div>
<style>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                            <input type="hidden" name="submitAllRec" id="submitAllRec" value="1"/>
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                ...
                                <tbody>
                                    <?php
										...
										$filterQry = "";
										if(isset($_REQUEST['grp_id']) && !is_array($_REQUEST['grp_id'])){
											$filterQry = " AND g.grp_id=".$_REQUEST['grp_id'];
										}
										if(isset($_REQUEST['contactid'])){
											$filterQry .= " AND c.ContactID=".$_REQUEST['contactid'];
										}
										$Query = "SELECT p.Pms_Package_ID, p.Package_Name, p.Arrival_Start_Date, p.Arrival_End_Date, p.Package_Max_Days, g.grp_id, g.GroupName, gc.ContactID, c.ContactFirstName, c.ContactLastName, c.cont_image
FROM packages AS p 
LEFT OUTER JOIN groups AS g ON g.Pms_Package_ID=p.Pms_Package_ID 
LEFT OUTER JOIN group_contacts AS gc ON gc.grp_id=g.grp_id 
LEFT OUTER JOIN contacts AS c ON c.ContactID=gc.ContactID 
WHERE p.Pms_Package_ID=".$_REQUEST['pms_pak_id']." AND p.Arrival_Start_Date > '2014-05-01' AND g.GroupArrivalDate > '2014-05-01' AND c.is_completed>=1 ".$filterQry."
ORDER BY g.GroupName, c.ContactFirstName ASC ";
                                        $counter = 0;
                                                        ...
                                                        if ($day_counter != 1) {
                                                            if ($no_of_days == $day_counter) {
                                                                ...
                                                                if ($no_of_days != (1 + $day_counter)) {
                                                                    ...
                                                                    $Query99 = " 
                                                                    SELECT 
                                                                    asch.asch_id, asch.act_id, asch.asch_start_date, asch.asch_end_date, acn.act_name, 
                                                                    asch.ag_id, asch.ad_id, asch.av_id, asch.av_confirmed, asch.av_confirming_person, asch.av_date, asch.av_time, asch.cont_id, asch.grp_id, asch.act_boat_id
                                                                    ...
                                                                    WHERE
                                                                    asch.cont_id = ".$row->ContactID." AND asch.act_id NOT IN (0,1) 
                                                                    ...
                                                                    ?><div class="scheduled">
                                                                        Activity: <br/> 
                                                                        <select name="act_id[]" id="asch_duration" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <option value="0"></option>
                                                                            <?php echo FillSelected("activities WHERE act_id NOT IN (1) ", "act_id", "act_name", $row99->act_id);?>
                                                                        </select>
                                                                        <br/> <br/> 
                                                                        
                                                                        Start Time: <br/> 
                                                                        <input class="timepicker" type="text" name="asch_start_time[]" value="<?php echo date("h:i A",strtotime($row99->asch_start_date));?>" style="width:200px;">
                                                                        <p> Time Format: 12 Hour System </p>
                                                                        
                                                                        End Time: <br/> 
                                                                        <input class="timepicker" type="text" name="asch_end_time[]" value="<?php echo date("h:i A",strtotime($row99->asch_end_date));?>" style="width:200px;">
                                                                        <p> Time Format: 12 Hour System </p>

                                                                        Guide: <br/> 
                                                                        <select name="ag_id[]" id="ag_id" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <option value="0"></option>
                                                                            <?php echo FillSelected("activity_guides", "ag_id", "ag_name", $row99->ag_id); ?>
                                                                        </select>
                                                                        <br/> <br/> 
                                                                        
                                                                        Vendor: <br/> 
                                                                        <select name="av_id[]" id="av_id" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <option value="0"></option>
                                                                            <?php echo FillSelected("activity_vendors", "av_id", "av_name", $row99->av_id); ?>
                                                                        </select>
                                                                        <br/> <br/> 

                                                                        Transport: <br/> 
                                                                        <select name="act_boat_id[]" id="act_boat_id" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <option value="0"></option>
                                                                            <?php echo FillSelected("act_boats", "act_boat_id", "act_boat_name", $row99->act_boat_id); ?>
                                                                        </select>
                                                                        <br/> <br/> 

                                                                        <div style="padding-left: 0px; padding-top:10px;">
                                                                            <strong><a href="manage_advent_activity_assignment.php?dell=1&pms_pak_id=<?php echo $_REQUEST['pms_pak_id'];?>&asch_id=<?php echo $row99->asch_id;?>">Delete This Record</a></strong>
                                                                        </div>
                                                                        <input type="hidden" value="<?php echo $row99->cont_id;?>" name="cont_id[]" >
                                                                        <input type="hidden" value="<?php echo $row99->grp_id;?>" name="grp_id[]" >
                                                                        <input type="hidden" value="<?php echo $dt->format("Y-m-d");?>" name="asch_start_date[]" >
                                                                        <input type="hidden" value="<?php echo $row99->asch_id;?>" name="asch_id[]" >
                                </div>
                                                                    <?php
                                                                        }
                                                                    } else {
                                                                    ?>
                                                                        ...
                                                                        Start Time: <br/> 
                                                                        <input class="timepicker" type="text" name="asch_start_time[]" value="09:00 AM" style="width:200px;">
                                                                        <p> Time Format: 12 Hour System </p>
                                                                        
                                                                        End Time: <br/> 
                                                                        <input class="timepicker" type="text" name="asch_end_time[]" value="05:00 PM" style="width:200px;">
                                                                        <p> Time Format: 12 Hour System </p>
                                                                        ...
                                                                        <input type="hidden" value="<?php echo $row->ContactID;?>" name="cont_id[]" >
                                                                        <input type="hidden" value="<?php echo $row->grp_id;?>" name="grp_id[]" >
                                                                        <input type="hidden" value="<?php echo $dt->format("Y-m-d");?>" name="asch_start_date[]" >
                                                                        <input type="hidden" value="0" name="asch_id[]" >
                                                                    <?php
                                                                   }
                                                                    echo '</td>';
                                                                }
                                    ...
                                    ?>
                                            <tr>
                                                <td style="vertical-align: middle; text-align: center;" colspan="100%">
                                                    <input type="submit" value="Submit" name="submitAllRec" id="submitAllRec" class="btn bg-primary text-white btn-lg">
                                                </td>
                                            </tr>
                                </tbody>
                            </table>
...
";
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-glass"></i> Manage Activities Assignment 
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
