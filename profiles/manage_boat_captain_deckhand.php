...
if(isset($_REQUEST['submitAllRec'])){
    ...
    echo '</pre>';
     ...
     mysql_query("DELETE FROM act_new_boats WHERE asch_id >0 OR asch_id = 0");
    for ($i = 0; $i < count($_REQUEST['actboca_id']); $i++) {
        $arr1 = explode(',',$_REQUEST['actboca_id'][$i]);
        $arr2 = explode(',',$_REQUEST['actbodh_id'][$i]);
            
        $oactboca_id        =   $arr1[0];
        $nactboca_id        =   $arr1[4];
        
        $asch_start_date    =   $arr1[1];
        $act_boat_id        =   $arr1[2];
        $asch_id            =   $arr1[3];
        ...
        if($oactboca_id != $nactboca_id){
            $exist = chkExist("asch_id", "act_new_boats", " WHERE asch_start_date = '".$asch_start_date."' AND act_boat_id = ".$act_boat_id."   ");
            if($exist > 0){
                mysql_query("UPDATE act_new_boats SET actboca_id = '".$oactboca_id."' WHERE asch_id = ".$asch_id."") or die(mysql_error());
            } else if($asch_id != '' && $asch_id != 0){
                mysql_query("INSERT INTO act_new_boats (asch_id, asch_start_date, act_boat_id, actboca_id) VALUES ('".$asch_id."', '".$asch_start_date."', '".$act_boat_id."', '".$oactboca_id."')");
            }    
        } else {
            $exist = chkExist("asch_id", "act_new_boats", " WHERE asch_start_date = '".$asch_start_date."' AND act_boat_id = ".$act_boat_id."   ");
            if($exist > 0){
        ...
        if($oactbodh_id != $nactbodh_id){
            $exist = chkExist("asch_id", "act_new_boats", " WHERE asch_id = ".$asch_id."");
            if($exist > 0){
                mysql_query("UPDATE act_new_boats SET actbodh_id = '".$oactbodh_id."' WHERE asch_id = ".$asch_id."") or die(mysql_error());
            } else if($asch_id != '' && $asch_id != 0){
                mysql_query("INSERT INTO act_new_boats (asch_id, asch_start_date, act_boat_id, actbodh_id) VALUES ('".$asch_id."', '".$asch_start_date."', '".$act_boat_id."', '".$oactbodh_id."')");
            }    
        } else {
            $exist = chkExist("asch_id", "act_new_boats", " WHERE asch_id = ".$asch_id."");
            if($exist > 0){
    ...
    if($asch_id == 0){
        ...
        $exist = chkExist("act_boat_id", "act_new_boats", " WHERE asch_start_date = '".$asch_start_date."' AND act_boat_id = ".$act_boat_id." AND asch_id = 0 ");
        ...
        if($exist > 0){
            echo '<br/>';
            echo "UPDATE act_new_boats SET actboca_id = '".$oactboca_id."' AND actbodh_id = ".$oactbodh_id." WHERE asch_start_date = '".$asch_start_date."' AND act_boat_id = ".$act_boat_id." AND asch_id = 0 ";

            mysql_query("UPDATE act_new_boats SET actboca_id = '".$oactboca_id."', actbodh_id = ".$oactbodh_id." WHERE asch_start_date = '".$asch_start_date."' AND act_boat_id = ".$act_boat_id." AND asch_id = 0 ") or die(mysql_error());
        } else {
            echo '<br/>';
            echo "INSERT INTO act_new_boats (asch_id, asch_start_date, act_boat_id, actboca_id, actbodh_id) VALUES ('0', '".$asch_start_date."', '".$act_boat_id."', '".$oactboca_id."', '".$oactbodh_id."')";
            
            mysql_query("INSERT INTO act_new_boats (asch_id, asch_start_date, act_boat_id, actboca_id, actbodh_id) VALUES ('0', '".$asch_start_date."', '".$act_boat_id."', '".$oactboca_id."', '".$oactbodh_id."')") or die(mysql_error());
        }
    ...
    header("Location: manage_boat_captain_deckhand.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=2");
}
if(isset($_REQUEST['dell'])){
    @mysql_query("DELETE FROM act_schedule  WHERE asch_id = " . $_REQUEST['asch_id']);
    @mysql_query("DELETE FROM act_new_boats  WHERE asch_id = " . $_REQUEST['asch_id']);
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Boat's Captain & Deckhand <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b> Manage Boat's Captain & Deckhand: </b> You can  Manage Boat's Captain & Deckhand here </p>
        </blockquote>
    </div>
</div>
                    ...
                    <div class="panel-body">
                        <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                <thead>
                                    <tr>
                                        <th class="visible-xs visible-sm visible-md visible-lg" style="max-width: 100px;">Boat Name</th>
                                        <?php
                                            $day_counter = 0;
                                            $day_counter1 = 0;
                                            $date1 = '';
                                ...
                                <tbody>
                                    <?php
                                        $counter = 0;
                                        $Query = "SELECT * FROM act_boats ORDER BY act_boat_name ";
                                    ...
                                    ?>
                                            <tr>
                                                <td class="visible-xs visible-sm visible-md visible-lg" style="max-width: 100px;"><?php echo $row->act_boat_name;?></td>
                                                <?php
                                                    $day_counter = 0;
                                                    $day_counter1 = 0;
                                                    $date1 = '';
                                                            ...
                                                            if ($no_of_days == $day_counter) {
                                                                ...
                                                                if ($no_of_days != (1 + $day_counter)) {
                                                                    ...
                                                                    //echo '<br>2 '.$dt->format("m/d/Y");
                                                                        ...
                                                                        $Query70 = " 
SELECT
asch.asch_id, asch.act_boat_id, asch.asch_start_date, ob.actboca_id AS oactboca_id, ob.actbodh_id AS oactbodh_id, 
nb.actboca_id AS nactboca_id, nb.actbodh_id AS nactbodh_id 

FROM 
act_schedule AS asch 
LEFT OUTER JOIN act_new_boats AS nb ON  asch.act_boat_id = nb.act_boat_id  AND nb.asch_start_date LIKE '".$dt->format("Y-m-d")."%' 
LEFT OUTER JOIN act_boats AS ob ON asch.act_boat_id = ob.act_boat_id

WHERE
asch.act_boat_id = ".$row->act_boat_id." AND asch.asch_start_date LIKE '".$dt->format("Y-m-d")."%'

AND ( nb.actboca_id  != '' OR  nb.actbodh_id != '' )
GROUP BY asch.act_boat_id


                                                                        ";
                                                                        $count70 = mysql_num_rows(mysql_query($Query70));
                                                                        $rs70 = mysql_query($Query70);
                                                                        if ($count70 > 0) {
                                                                            while ($row70 = mysql_fetch_object($rs70)) {
                                                                                $counter70++;
                                                                        ?>
                                                                                Captain: <br/> 
                                                                                <select name="actboca_id[]" data-placeholder="Choose a Captain..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                                    <?php
                                                                                        $strQueryv = "SELECT actboca_id, actboca_name FROM act_boat_captain ORDER BY actboca_name";
                                                                                        $nResultv = mysql_query($strQueryv);
                                                                                        if (mysql_num_rows($nResultv) >= 1) {
                                                                                            while ($rowv = mysql_fetch_row($nResultv)) {
                                                                                                if($row70->nactboca_id != ''){
                                                                                                    if($rowv[0] == $row70->nactboca_id){
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.','.$row70->nactboca_id." selected>".$rowv[1]."</option>";
                                                                                                    } else {
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.',0'.">".$rowv[1]."</option>";
                                                                                                    }
                                                                                                } else if($row70->oactboca_id != ''){
                                                                                                    if($rowv[0] == $row70->oactboca_id){
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.','.$row70->oactboca_id." selected>".$rowv[1]."</option>";
                                                                                                    } else {
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.',0'.">".$rowv[1]."</option>";
                                                                                                    }
                                                                                                } else {
                                                                                ...
                                                                                <br/><br/>
                                                                                Deckhand: <br/> 
                                                                                <select name="actbodh_id[]" data-placeholder="Choose a Deckhand..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                                    <?php
                                                                                        $strQueryv = "SELECT actbodh_id, actbodh_name FROM act_boat_deckhand ORDER BY actbodh_name";
                                                                                        $nResultv = mysql_query($strQueryv);
                                                                                        if (mysql_num_rows($nResultv) >= 1) {
                                                                                            while ($rowv = mysql_fetch_row($nResultv)) {
                                                                                                if($row70->nactbodh_id != ''){
                                                                                                    if($rowv[0] == $row70->nactbodh_id){
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.','.$row70->nactbodh_id." selected>".$rowv[1]."</option>";
                                                                                                    } else {
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.',0'.">".$rowv[1]."</option>";
                                                                                                    }
                                                                                                } else if($row70->oactbodh_id != ''){
                                                                                                    if($rowv[0] == $row70->oactbodh_id){
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.','.$row70->oactbodh_id." selected>".$rowv[1]."</option>";
                                                                                                    } else {
                                                                                                        echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->act_boat_id.','.$row70->asch_id.',0'.">".$rowv[1]."</option>";
                                                                                                    }
                                                                                                } else {
...
FROM 
act_schedule AS asch 
LEFT OUTER JOIN act_new_boats AS nb ON  asch.act_boat_id = nb.act_boat_id  AND nb.asch_start_date LIKE '".$dt->format("Y-m-d")."%' AND asch.asch_id = nb.asch_id 
LEFT OUTER JOIN act_boats AS ob ON asch.act_boat_id = ob.act_boat_id

WHERE
asch.act_boat_id = ".$row->act_boat_id." AND asch.asch_start_date LIKE '".$dt->format("Y-m-d")."%'

AND ( nb.actboca_id IS NULL OR  nb.actbodh_id IS NULL )
LIMIT 1
                                                                        ";
                                                                        $count70 = mysql_num_rows(mysql_query($Query70));
                                                                        $rs70 = mysql_query($Query70);
...
<!--                                                                            <br/>Exceptional<br/>-->
                                                                            ...
                                                                            <select name="actboca_id[]" data-placeholder="Choose a Captain..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                                <?php
                                                                                    echo $strQueryv = "  
SELECT 
abc.actboca_id, abc.actboca_name, actb.actboca_id AS oactboca_id, 
acnbc.actboca_id AS nbc, 
actb.act_boat_id 
FROM 
act_boat_captain AS abc
LEFT OUTER JOIN act_boats AS actb ON abc.actboca_id = actb.actboca_id  
LEFT OUTER JOIN act_new_boats AS acnbc  ON actb.act_boat_id = acnbc.act_boat_id AND acnbc.asch_id = 0 AND acnbc.asch_start_date LIKE '".$dt->format("Y-m-d")."%'   AND acnbc.act_boat_id = ".$row->act_boat_id." 
GROUP BY 
abc.actboca_id 
ORDER BY 
acnbc.actboca_id DESC 
";
                                                                                    $nResultv = mysql_query($strQueryv);
                                                                                    if (mysql_num_rows($nResultv) >= 1) {
                                                                                        while ($rowv = mysql_fetch_object($nResultv)) {
                                                                                            if($rowv->nbc != ''){
                                                                                                $nbc = $rowv->nbc;
                                                                                                echo "<option value=".$rowv->nbc.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->nbc.',0,0'." selected>".  returnName("actboca_name", "act_boat_captain", "actboca_id", $rowv->nbc) ."</option>";
                                                                                                echo "<option value=".$rowv->actboca_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actboca_id.',0,0'." >".$rowv->actboca_name."</option>";
                                                                                            } else if($rowv->actboca_id != @$nbc){
                                                                                                 $oldc_id = returnName("actboca_id", "act_boats", "act_boat_id", $row->act_boat_id);
                                                                                                 if($rowv->actboca_id == $oldc_id){
                                                                                                    echo "<option value=".$rowv->actboca_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actboca_id.',0,0'." selected>".$rowv->actboca_name."</option>";
                                                                                                 } else {
                                                                                                    echo "<option value=".$rowv->actboca_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actboca_id.',0,0'." >".$rowv->actboca_name."</option>";
                                                                                                 }
                                                                                            }
                                                                            ...
                                                                            <select name="actbodh_id[]" data-placeholder="Choose a Deckhand..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                                <?php
                                                                                   echo  $strQueryv = "  
SELECT 
abd.actbodh_id, abd.actbodh_name, actb.actbodh_id AS oactbodh_id, 
acnbd.actbodh_id AS nbd, 
actb.act_boat_id 
FROM 
act_boat_deckhand AS abd
LEFT OUTER JOIN act_boats AS actb ON abd.actbodh_id = actb.actbodh_id  
LEFT OUTER JOIN act_new_boats AS acnbd  ON actb.act_boat_id = acnbd.act_boat_id AND acnbd.asch_id = 0 AND acnbd.asch_start_date LIKE '".$dt->format("Y-m-d")."%' AND acnbd.act_boat_id = ".$row->act_boat_id." 
GROUP BY 
abd.actbodh_id 
ORDER BY 
acnbd.actbodh_id DESC 
";
                                                                                    $nResultv = mysql_query($strQueryv);
                                                                                    if (mysql_num_rows($nResultv) >= 1) {
                                                                                        while ($rowv = mysql_fetch_object($nResultv)) {
                                                                                            ...
                                                                                            if($rowv->nbd != ''){
                                                                                                $nbd = $rowv->nbd;
                                                                                                echo "<option value=".$rowv->nbd.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->nbd.',0,0'." selected>".  returnName("actbodh_name", "act_boat_deckhand", "actbodh_id", $rowv->nbd) ."</option>";
                                                                                                echo "<option value=".$rowv->actbodh_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actbodh_id.',0,0'." >".$rowv->actbodh_name."</option>";
                                                                                            } else if($rowv->actbodh_id != @$nbd){
                                                                                                 $oldd_id = returnName("actbodh_id", "act_boats", "act_boat_id", $row->act_boat_id);
                                                                                                 if($rowv->actbodh_id == $oldd_id){
                                                                                                    echo "<option value=".$rowv->actbodh_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actbodh_id.',0,0'." selected>".$rowv->actbodh_name."</option>";
                                                                                                 } else {  
                                                                                                    echo "<option value=".$rowv->actbodh_id.','.$dt->format("Y-m-d").','.$row->act_boat_id.',0,'.$rowv->actbodh_id.',0,0'." >".$rowv->actbodh_name."</option>";
                                                                                                 }
                                                                                            }
...
";
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-glass"></i> Manage Boat's Captain & Deckhand 
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
		...
		$Query = "SELECT p.* FROM packages AS p WHERE  p.Arrival_Start_Date > '".date("Y")."-01-01' ORDER BY p.Arrival_Start_Date ASC ";
        $counter = 0;
