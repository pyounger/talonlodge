...
<?php } else { ?>
    <div style="visibility: hidden; display: none;">
        ...
        cp.conp_id, cp.bootsize_id, cac.act_id
        FROM packages AS p 
        LEFT OUTER JOIN groups AS g ON g.Pms_Package_ID=p.Pms_Package_ID 
        LEFT OUTER JOIN contacts AS c ON c.ContactID=g.Contact_ID
        LEFT OUTER JOIN contact_profiles AS cp ON cp.cont_id=c.ContactID
        LEFT OUTER JOIN contact_activities AS cac ON cac.conp_id=cp.conp_id
        WHERE g.grp_id>0 AND p.Arrival_Start_Date > '" . $_SESSION['pk_from'] . "'  AND c.is_completed>=1
        GROUP BY g.grp_id ORDER BY p.Arrival_Start_Date ASC";

        $limit1 = $_SESSION['limit_of_rec'];
        $start1 = $p->findStart($limit1);
        $queryResult1 = mysql_query($Query1);
        $count1 = mysql_num_rows($queryResult1);
        ...
        if ($count1 > 0) {
            while ($row1 = mysql_fetch_array($rs1)) {
                ...
                //$Query2 = "SELECT c.ContactID, c.grp_id, c.ContactFirstName, c.ContactLastName FROM contacts AS c WHERE c.grp_id=" . $row1->grp_id . " ";
                //$Query2 = "SELECT c.ContactID, c.grp_id, c.ContactFirstName, c.ContactLastName FROM contacts AS c WHERE c.grp_id=" . $row1->grp_id . " ";
                $Query2 = "SELECT gc.grp_id, c.ContactID, c.ContactFirstName, c.ContactLastName FROM group_contacts AS gc "
                        . "LEFT OUTER JOIN contacts AS c ON c.ContactID=gc.ContactID WHERE gc.grp_id=" . $row1['grp_id'];
                ...
                if ($count2 > 0) {
                    ...
                    <table class="table users-table table-condensed table-hover table-striped display dataTable">
                        ...
                        while ($row2 = mysql_fetch_array($queryResult2)) {
                            ...
                            $total_completed = 0;
                            $counter3 = 0;
                            $Query3 = "SELECT cp.conp_id, cp.cont_id, ca.act_id, a.act_name, "
                                    . "g.grp_id, g.GroupName, c.cont_fname, c.cont_lname, "
                                    . "asch.asch_id, asch.cont_id AS aschcont_id, "
                                    . "asch.asch_start_date, asch.asch_end_date, "
                                    . "asch.asch_duration, asch.asch_email_counter, ag.ag_name, "
                                    . "ad.ad_name, av.av_name, abn.act_boat_name, thr.ath_id, "
                                    . "thr.ath_name, thrt.atht_id, thrt.atht_name "
                                    . "FROM  contact_profiles AS cp "
                                    . "LEFT OUTER JOIN contact_activities AS ca ON cp.conp_id=ca.conp_id "
                                    . "LEFT OUTER JOIN activities AS a ON ca.act_id=a.act_id "
                                    . "LEFT OUTER JOIN act_schedule AS asch ON asch.grp_id=" . $row1['grp_id'] . " AND asch.cont_id=" . $row2['ContactID'] . " AND asch.act_id=a.act_id "
                                    . "LEFT OUTER JOIN groups AS g ON g.grp_id=" . $row1['grp_id'] . " "
                                    . "LEFT OUTER JOIN contacts AS c ON c.ContactID=" . $row2['ContactID'] . " "
                                    . "LEFT OUTER JOIN activity_guides AS ag ON asch.ag_id=ag.ag_id "
                                    . "LEFT OUTER JOIN activity_destination AS ad ON asch.ad_id=ad.ad_id "
                                    . "LEFT OUTER JOIN activity_vendors AS av ON asch.av_id=av.av_id "
                                    . "LEFT OUTER JOIN act_therapist AS thr ON asch.ath_id=thr.ath_id "
                                    . "LEFT OUTER JOIN act_th_type AS thrt ON asch.atht_id=thrt.atht_id "
                                    . "LEFT OUTER JOIN act_boats AS abn ON asch.act_boat_id=abn.act_boat_id "
                                    . "WHERE ( cp.cont_id=" . $row2['ContactID'] . " OR  asch.cont_id=" . $row2['ContactID'] . " ) "
                                    . "AND a.act_id !='' GROUP BY a.act_id ORDER BY a.act_order ASC";
                             $queryResult3 = mysql_query($Query3);       
                            $count3 = mysql_num_rows($queryResult3);
                            if ($count3 > 0) {
                                ?>
                                <?php
                                while ($row3 = mysql_fetch_array($queryResult3)) {
                                    $counter3++;
                                    ?>
                                                                                                                                                                                                                                    <!--                                                                        <tr>                                                                                                                                                                         <td class="visible-xs visible-sm visible-md visible-lg"><?php echo $row3->act_name; ?> </td>                                                                                                                                                                                                                                    <td class="visible-xs visible-sm visible-md visible-lg">
                                    <?php
                                    if ($row3['asch_id'] != 0 || $row3['asch_id'] != '') {
                                        $total_completed++;
                                        $compl += 1;
                    ...
                    <div class="row">
                        <div class="">
                            <ul class="nav faq-list">
                                ...
                                /* ORDER BY g.GroupName, c.ContactFirstName ASC"; */
                                //print("Total: ".count($pkg_completed)."<br>");
                                //print_r($pkg_completed);
                                //print($Query1);
                                $count1 = 0;
                                $limit1 = $_SESSION['limit_of_rec'];
                                $start1 = $p->findStart($limit1);
                                $query1result = mysql_query($Query1);
                                $count1 = mysql_num_rows($query1result);
                                $pages1 = $p->findPages($count1, $limit1);
                                $rs1 = mysql_query($Query1 . " LIMIT " . $start1 . ", " . $limit1);
                                if ($count1 > 0) {
                                    while ($row1 = mysql_fetch_array($rs1)) {
                                        ...
                                        //$Query2 = "SELECT c.ContactID, c.grp_id, c.ContactFirstName, c.ContactLastName FROM contacts AS c WHERE c.grp_id=" . $row1->grp_id . " ";
                                        //$Query2 = "SELECT gc.grp_id, c.ContactID, c.ContactFirstName, c.ContactLastName FROM group_contacts AS gc LEFT OUTER JOIN contacts AS c ON c.ContactID=gc.ContactID WHERE gc.grp_id=".$row1->grp_id;
                                        $Query2 = "SELECT gc.grp_id, c.ContactID, c.ContactFirstName, c.ContactLastName "
                                                . "FROM group_contacts AS gc, contacts AS c WHERE c.ContactID = gc.ContactID "
                                                . "AND gc.grp_id=" . $row1['grp_id'] . " GROUP BY c.cont_id ";
                                        //print($Query2);
                                        $rs2 = mysql_query($Query2);
                                        ...
                                        if ($count2 > 0) {
                                            ...
                                            if ($row1['Pms_Package_ID'] == trim($arr[0])) {
                                                //echo '<br/>'. $arr[0] .' '. $arr[1];
                                                if (trim($arr[1]) == 0) {
                                                    ...
                                                    <li id="collapse<?php echo $row1['grp_id'] ?>" class="panel-collapse collapse" style="list-style-type:none;">
                                                        ...
                                                        <table class="table users-table table-condensed table-hover table-striped display dataTable">
                                                            ...
                                                            while ($row2 = mysql_fetch_array($rs2)) {
                                                                ...
                                                                $counter3 = 0;
                                                                $Query3 = "SELECT cp.conp_id, cp.cont_id, ca.act_id, a.act_name, g.grp_id, g.GroupName, c.cont_fname, c.cont_lname, asch.asch_id, asch.cont_id AS aschcont_id, asch.asch_start_date, asch.asch_end_date, asch.asch_duration, asch.asch_email_counter, ag.ag_name, ad.ad_name, av.av_name, abn.act_boat_name, thr.ath_id, thr.ath_name, thrt.atht_id, thrt.atht_name FROM  
																contact_profiles AS cp 
																LEFT OUTER JOIN contact_activities AS ca ON cp.conp_id=ca.conp_id 
																LEFT OUTER JOIN activities AS a ON ca.act_id=a.act_id 
																LEFT OUTER JOIN act_schedule AS asch ON asch.grp_id=" . $row1['grp_id'] . " 
																AND asch.cont_id=" . $row2['ContactID'] . " 
																AND asch.act_id=a.act_id 
																LEFT OUTER JOIN groups AS g ON g.grp_id=" . $row1['grp_id'] . " 
																LEFT OUTER JOIN contacts AS c ON c.ContactID=" . $row2['ContactID'] . " 
																LEFT OUTER JOIN activity_guides AS ag ON asch.ag_id=ag.ag_id 
																...
																LEFT OUTER JOIN act_th_type AS thrt ON asch.atht_id=thrt.atht_id 
																LEFT OUTER JOIN act_boats AS abn ON asch.act_boat_id=abn.act_boat_id 
																WHERE ( cp.cont_id=" . $row2['ContactID'] . " OR  asch.cont_id=" . $row2['ContactID'] . " ) AND a.act_id !='' GROUP BY a.act_id ORDER BY a.act_order ASC";
                                                                                        ...
                                                                                        if ($no_of_days == $day_counter) {
                                                                                            ...
                                                                                            if ($no_of_days != (1 + $day_counter)) {
                                                                                                ...
                                                                                                if ($row3['asch_id'] != 0 || $row3['asch_id'] != '') {
                                                                                                    ...
                                                                                                    $count5 = 0;
                                                                                                    $counter5 = 0;
                                                                                                    $Query5 = "SELECT asch.asch_id FROM act_schedule AS asch WHERE asch.act_id=" . $row3['act_id'] . " AND asch.cont_id = " . $row3['aschcont_id'] . " AND asch.grp_id = " . $row3['grp_id'] . " AND asch.asch_start_date LIKE '" . $dt->format("Y-m-d") . "%' LIMIT 1";
                                                                                                    $rs5 = mysql_query($Query5);
                                                                                                    $count5 = mysql_num_rows($rs5);
                                                                                                    if ($count5 > 0) {
                                                                                                        while ($row5 = mysql_fetch_array($rs5)) {
...
//                                                                                                            - <div class='tooltips'><a href=".$_SERVER['PHP_SELF'] . "?send=1&cont_id=" . $row3->aschcont_id ."&act_id=" . $row3->act_id."&asch_id=" . $row3->asch_id." data-original-title= 'Email has been sent ".$row3->asch_email_counter.(($row3->asch_email_counter>1)?" times ":" time ")."' data-placement='top' class='btn bg-primary text-white' style='float:left; margin-left: 2px; margin-bottom: 2px;'><i class='fa fa-envelope-o'></i></a></div>";
                                                                                                        ...
                                                                                                        //echo $dt->format("m/d/Y");
                                                                                                        echo "<a href=" . $_SERVER['PHP_SELF'] . "?action=1&sch=0&cont_id=" . (($row3['aschcont_id'] != '') ? $row3['aschcont_id'] : $row3['cont_id']) . "&grp_id=" . $row3['grp_id'] . "&act_id=" . $row3['act_id'] . "&asch_id=&pms_pak_id=" . $row1['Pms_Package_ID'] . ">Not Scheduled</a>";
                                                                                                    }
                                                                                                } else {
                                                                                                    echo "<a href=" . $_SERVER['PHP_SELF'] . "?action=1&sch=0&cont_id=" . (($row3['aschcont_id'] != '') ? $row3['aschcont_id'] : $row3['cont_id']) . "&grp_id=" . $row3['grp_id'] . "&act_id=" . $row3['act_id'] . "&asch_id=&pms_pak_id=" . $row1['Pms_Package_ID'] . ">Not Scheduled</a>";
                                                                                                }
