...
if (isset($_REQUEST['btnAdd'])) {
    $isexist = chkExist("acco_id", "activity_confirmation", " WHERE cont_id=".$_REQUEST['cont_id']." AND grp_id=".$_REQUEST['grp_id']." AND pms_pak_id=".$_REQUEST['pms_pak_id']." AND asch_id=".$_REQUEST['asch_id']." AND act_id=".$_REQUEST['act_id']." ");
    if(isset($_REQUEST['acco_completed'])){
        $acco_completed = '1';
    } else {
        $acco_completed = '0';
    }
    if($isexist>0){
        mysql_query("UPDATE activity_confirmation SET ath_id='" . @dbStr(@($_REQUEST['ath_id'])) . "', atht_id='" . @dbStr(@($_REQUEST['atht_id'])) . "', ag_id='" . @dbStr(@($_REQUEST['ag_id'])) . "', acco_completed='" . @dbStr(@($acco_completed)) . "', acco_comments='" . @dbStr(@($_REQUEST['acco_comments'])) . "', acco_consumed='" . @dbStr(@($_REQUEST['acco_consumed'])) . "'  WHERE acco_id=".$_REQUEST['acco_id']."") or die(mysql_error());
        header("Location: manage_confirmations.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=2");
    } else {
        $maxID = getMaximum("activity_confirmation", "acco_id");
        mysql_query("INSERT INTO activity_confirmation (acco_id, asch_id, cont_id, grp_id, act_id, ath_id, atht_id, ag_id, pms_pak_id, acco_date, acco_completed, acco_comments, acco_consumed) VALUES(" . $maxID . ", '" . dbStr($_REQUEST['asch_id']) . "', '" . dbStr($_REQUEST['cont_id']) . "', '" . dbStr($_REQUEST['grp_id']) . "', '" . @dbStr(@$_REQUEST['act_id']) . "', '" . @dbStr(@$_REQUEST['ath_id']) . "', '" . @dbStr(@($_REQUEST['atht_id'])) . "', '" . @dbStr(@($_REQUEST['ag_id'])) . "', '" . @dbStr(@($_REQUEST['pms_pak_id'])) . "', '" . @$_REQUEST['date'] . "', '" . @dbStr(@($acco_completed)) . "', '" . @dbStr(@($_REQUEST['acco_comments'])) . "', '" . @dbStr(@($_REQUEST['acco_consumed'])) . "')") or die(mysql_error());
        header("Location: manage_confirmations.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=1");
    }    
} 
...
if (isset($_REQUEST['action'])) {
    if ($_REQUEST['action'] == 2) {
        ...
        if (mysql_num_rows($rsM) > 0) {
            ...
            $acco_consumed = $rs->acco_consumed;
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Confirmations <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Confirmations: </b> You can manage your Confirmations here </p>
        </blockquote>
    </div>
</div>
                                                            ...
                                                            if ($day_counter != 1) {
                                                                if ($no_of_days == $day_counter) {
                                                                    ...
                                                                    if ($no_of_days != (1+ $day_counter)) {
                                                                        $Query1 = "
SELECT 
acts.asch_id, acts.act_id, ac.act_name, acc.acco_id
FROM 
act_schedule AS acts 
LEFT OUTER JOIN activities AS ac ON acts.act_id=ac.act_id 
LEFT OUTER JOIN activity_confirmation AS acc ON acts.cont_id=acc.cont_id AND acts.grp_id=acc.grp_id AND acts.act_id=acc.act_id AND acts.asch_id=acc.asch_id
WHERE 
...
AND acts.grp_id=".$row->grp_id." ";
                                                                        ...
                                                                        if ($count1 > 0) {
                                                                            while ($row1 = mysql_fetch_object($rs1)) {
                                                                                $day_counter1++;
                                                                                if($row1->acco_id!=''){
                                                                                    echo $row1->act_name . '<br/>' . '<a href=' . $_SERVER['PHP_SELF'] . "?show=2&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id."&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id."&add=1". '>Confirmed</a>' . '<br/><a href=' . $_SERVER['PHP_SELF'] . "?show=3&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id.'>Details</a>';
                                                                                } else {
                                                                                    echo $row1->act_name . '<br/>' . '<a href=' . $_SERVER['PHP_SELF'] . "?show=2&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id."&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id."&add=1". '>Set Confirmation</a>';
                                                                                }
                                                                                echo '<br/><br/>';
                                                                            }
                                                                        } else {
...
AND acts.grp_id=".$row->grp_id." ";
                                                                        ...
                                                                        if ($count1 > 0) {
                                                                            while ($row1 = mysql_fetch_object($rs1)) {
                                                                                $day_counter1++;
                                                                                if($row1->asch_id != ''){
                                                                                    echo $row1->act_name . '<br/>' . '<a href=' . $_SERVER['PHP_SELF'] . "?show=2&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id."&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id."&add=1". '>Confirmed</a>' . '<br/><a href=' . $_SERVER['PHP_SELF'] . "?show=3&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id.'>Details</a><br/>';
                                                                                } else {
                                                                                    echo $row1->act_name . '<br/>' . '<a href=' . $_SERVER['PHP_SELF'] . "?show=2&asch_id=" . $row1->asch_id . "&act_id=".$row1->act_id."&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id."&contactid=" . $row->ContactID . "&date=".$dt->format("Y-m-d")."&acco_id=".@$row1->acco_id."&add=1". '>Set Confirmation</a>';
                                                                                }
                                                                            }
        ...
        if(isset($_REQUEST['acco_id']) && $_REQUEST['acco_id']!=''){
            $acco_id = $_REQUEST['acco_id'];
        } else {
            $acco_id = 0;
        }
        $Query = "SELECT * FROM activity_confirmation WHERE acco_id = ".$acco_id." ";
        $rsM = mysql_query($Query);
        if (mysql_num_rows($rsM) > 0) {
            $rsMem = mysql_fetch_object($rsM);
            $acco_id = $rsMem->acco_id;
            ...
            $atht_id = $rsMem->atht_id;
            $ag_id = $rsMem->ag_id;
            $pms_pak_id = $rsMem->pms_pak_id;
            $acco_date = $rsMem->acco_date;
            $acco_completed = $rsMem->acco_completed;
            $acco_comments = $rsMem->acco_comments;
            $acco_consumed = $rsMem->acco_consumed;
        } else {
            $acco_id = "";
            $asch_id = "";
            $cont_id = "";
            ...
            <div class="col-md-12">
                <div class="panel panel-cascade">
                    ...
                    <div class="panel-body ">
                        <div class="ro">
                            <div class="col-mol-md-offset-2">
                                <form name="frm" id="frm" class="form-horizontal cascde-forms" method="post" action="<?php print($_SERVER['PHP_SELF']); ?>" name="frm" id="frm" >
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Completed:</label>
                                        <div class="col-lg-10 col-md-9">
                                            Yes <input type="radio" class="required" id="acco_completed" name="acco_completed" <?php echo ((@$acco_completed=='1')?"checked='checked'":'');?> style="width: 22px; height: 22px;"> &nbsp; 
                                            No  <input type="radio" class="required" id="acco_completed" name="acco_completed" <?php echo ((@$acco_completed=='0')?"checked='checked'":'');?> style="width: 22px; height: 22px;">
                                        </div>
                                    </div>
                                    <div class="form-group" id="set_guides">
                                        <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Completion Comments</label>
                                        <div class="col-lg-10 col-md-9">
                                            <textarea type="text" class="form-control form-cascade-control input_wid70" placeholder="Activity Completion Comments.." id="acco_comments" name="acco_comments"><?php echo $acco_comments;?></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group" id="set_guides">
                                        <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Duration (Minutes)</label>
                                        <div class="col-lg-10 col-md-9">
                                            <input type="text" class="form-control form-cascade-control input_wid70" placeholder="Activity Duration.." id="acco_consumed" name="acco_consumed" value="<?php echo $acco_consumed;?>" /> 
                                        </div>
                                    </div>
                                    <?php 
                                    ...
                                    ?>
                                    <div class="form-group" id="set_guides">
                                        <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Therapist</label>
                                        <div class="col-lg-10 col-md-9">
                                            <select data-placeholder="Choose a Therapist..." name="ath_id" id="ath_id" class="chosen-select required" style="width:240px; z-index: 9999 !important;" tabindex="2">
                                                <?php FillSelected("act_therapist", "ath_id", "ath_name", $ath_id); ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group" id="set_guides">
                                        <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Therapy Type</label>
                                        <div class="col-lg-10 col-md-9">
                                            <select data-placeholder="Choose a Therapy Type..." name="atht_id" id="atht_id" class="chosen-select required" style="width:240px; z-index: 9999 !important;" tabindex="2">
                                    ...
                                    <div class="form-actions">
                                        ...
                                        <input type="hidden" name="acco_id" value="<?php echo @$_REQUEST['acco_id']; ?>">

                                        <?php if ( !(isset($_REQUEST['acco_id']) && $_REQUEST['acco_id']!='') && (isset($_REQUEST['add'])) ) { ?>
                                            &nbsp; <input type="submit" value="Add Now" name="btnAdd" class="btn bg-primary text-white btn-lg">
                                        <?php } else { ?>
        ...
        <?php
                ...
                acco.acco_id, acco.acco_completed, acco.acco_comments, acco.acco_date, acco.acco_consumed, 
                c.ContactFirstName, c.ContactLastName, g.GroupName, p.Package_Name, ac.act_name
                FROM activity_confirmation AS acco
                LEFT OUTER JOIN contacts AS c ON acco.cont_id=c.ContactID
                LEFT OUTER JOIN groups AS g ON acco.grp_id=g.grp_id
                LEFT OUTER JOIN packages AS p ON acco.pms_pak_id=p.Pms_Package_ID
                LEFT OUTER JOIN activities AS ac ON acco.act_id=ac.act_id
                WHERE acco.acco_id=".$_REQUEST['acco_id']);
                if(mysql_num_rows($rsM)>0){
                    $rsMem = mysql_fetch_object($rsM);
        ?>
        <div class="row">
                ...
                <div class="panel panel-cascade">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Activity Date</label>
                            <div class="col-lg-10 col-md-9 det-display">
                                &nbsp; <?php echo calendarDateConver2($rsMem->acco_date); ?>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Completed</label>
                            <div class="col-lg-10 col-md-9 det-display">
                                &nbsp; <?php echo (($rsMem->acco_completed==1)?'Yes':'No');?>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Comments</label>
                            <div class="col-lg-10 col-md-9 det-display">
                                &nbsp; <?php echo $rsMem->acco_comments;?>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Consumed Time</label>
                            <div class="col-lg-10 col-md-9 det-display">
                                &nbsp; <?php echo $rsMem->acco_consumed;?> Minutes
                            </div>
                        </div>
                        <div class="form-group">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-cascade">
                    ...
                    <div class="panel-body">
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label"> Information Does Not Exists </label>
                            <div class="col-lg-10 col-md-9 det-display">  Information Does Not Exists </div>
                        </div>
                        <div class="form-group">
                            <label for="inputEmail1" class="col-lg-2 col-md-3 control-label">&nbsp;</label>
                            <div class="col-lg-10 col-md-9">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-glass"></i> Confirmations
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
                    <div class="panel-body">
                        <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="example" >
                                ...
                                <tbody>
        <?php
        $Query = "SELECT p.* FROM packages AS p WHERE p.Arrival_Start_Date > '2014-05-01' ORDER BY YEAR(p.Arrival_Start_Date) DESC,p.Arrival_Start_Date ASC";
        $counter = 0;
        $limit = $_SESSION['limit_of_rec'];
        $start = $p->findStart($limit);
        $count = mysql_num_rows(mysql_query($Query));
