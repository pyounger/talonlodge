...
if(isset($_REQUEST['submitAllRec'])){
    ...
    echo " save routine started. ";
    
    
    
    for ($i = 0; $i < count($_REQUEST['ath_id']); $i++) {
        
        //echo "<br />$i: ";
        if($_REQUEST['ath_id'][$i] != 0){
            if($_REQUEST['asch_id'][$i] != 0){
                $asch_start_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $asch_end_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $duration = ' + '.$_REQUEST['asch_duration'][$i].' minute ';
                ...
                $sql = "UPDATE act_schedule SET ath_id = '" . $_REQUEST['ath_id'][$i] . "', atht_id = '" . $_REQUEST['atht_id'][$i] . "', asch_start_date = '" . $asch_start_date . "', asch_end_date = '" . $asch_end_date . "', asch_duration = '" . $_REQUEST['asch_duration'][$i] . "', updatedDate = NOW() WHERE asch_id = ".$_REQUEST['asch_id'][$i]." ";
                //echo $sql;
                mysql_query($sql) or die(mysql_error());
                
                //header("Location: manage_massage_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=2");
            } else {
                $asch_start_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $asch_end_date = trim((dbStr($_REQUEST['asch_start_date'][$i])) . ' ' . $_REQUEST['asch_start_time'][$i]);
                $duration = ' + '.$_REQUEST['asch_duration'][$i].' minute ';
                ...
                $sql = "INSERT INTO act_schedule (asch_id, cont_id, grp_id, act_id, ath_id, atht_id, asch_start_date, asch_end_date, asch_duration, createdDate, pms_pak_id) VALUES (".$MaxID.", '".$_REQUEST['cont_id'][$i]."', '".$_REQUEST['grp_id'][$i]."', '1', '".$_REQUEST['ath_id'][$i]."', '".$_REQUEST['atht_id'][$i]."', '".$asch_start_date."', '".$asch_end_date."', '".$_REQUEST['asch_duration'][$i]."', NOW(), '".$_REQUEST['pms_pak_id']."')";
                //echo $sql;
                mysql_query($sql);
                //header("Location: manage_massage_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=1");
            }
        }
    }
...
if(isset($_REQUEST['dell'])){
    ...
    header("Location: manage_massage_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=3");
}
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Massage Assignment <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b> Massage Assignment </b> You can Manage Massage Assignment   here </p>
        </blockquote>
    </div>
</div>
<style>
                            ...
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                ...
                                <tbody>
                                    <?php
										...
										$filterQry = "";
										if(isset($_REQUEST['grp_id'])){
										     if(is_array($_REQUEST['grp_id']))
										     	{
												$filterQry = " AND g.grp_id=".$_REQUEST['grp_id'][0];
										     	}
										     	else
										     	{
                                                        ...
                                                        if ($day_counter != 1) {
                                                            if ($no_of_days == $day_counter) {
                                                                ...
                                                                if ($no_of_days != (1 + $day_counter)) {
                                                                    ...
                                                                    $Query99 = " 
SELECT 
asch.asch_id, asch.act_id, asch.asch_start_date, acn.act_name, 
asch.asch_duration, asch.ath_id, asch.atht_id, asch.cont_id, asch.grp_id 
...
WHERE
asch.cont_id = ".$row->ContactID." AND asch.act_id = 1 
...
";
                                                                    ...
                                                                    if ($count99 > 0) {
                                                                        ...
                                                                        <br /><br />
                                                                        Duration: <br/> 
                                                                        <select data-placeholder="Duration..." name="asch_duration[]" id="asch_duration" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <option value="30" <?php echo (($row99->asch_duration == '30') ? "selected" : '') ?>> 30 Minutes </option>
                                                                            <option value="50" <?php echo (($row99->asch_duration == '50' || $row99->asch_duration == '') ? "selected" : '') ?>> 50 Minutes </option>
                                                                            <option value="80" <?php echo (($row99->asch_duration == '80') ? "selected" : '') ?>> 80 Minutes </option>
                                                                        </select>
                                                                        <br/> <br/> 
                                                                        
                                                                        Therapist: <br/> 
                                                                        <select data-placeholder="Choose a Therapist..." name="ath_id[]" id="ath_id" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <?php FillSelected("act_therapist", "ath_id", "ath_name", $row99->ath_id); ?>
                                                                        </select>
                                                                        <br/>  <br/> 

                                                                        Therapy: <br/> 
                                                                        <select data-placeholder="Choose a Therapy Type..." name="atht_id[]" id="atht_id" class="chosen-" style="width:200px; z-index: 9999 !important;" tabindex="2">
                                                                            <?php echo FillSelected("act_th_type", "atht_id", "atht_name", $row99->atht_id); ?>
                                                                        </select>
                                                                        <div style="padding-left: 0px; padding-top:10px;">
                                                                            <strong><a href="manage_massage_assignment.php?dell=1&pms_pak_id=<?php echo $_REQUEST['pms_pak_id'];?>&asch_id=<?php echo $row99->asch_id;?>">Delete This Record</a></strong>
                                                                        </div>
                                                                    ...
                                                                    ?>
                                                                        Start Time: <br/> 
                                                                        <input class="timepicker" type="text" name="asch_start_time[]" value="" style="width:200px;">
                                                                        <br /><br />
    ...
    <div class="row">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-glass"></i> Manage Massage Assignment 
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
