...
if (isset($_REQUEST['reserveRoom'])) {
    ...
    if (isset($_REQUEST['chkstatus'])) {
        for ($i = 0; $i < count($_REQUEST['chkstatus']); $i++) {
            ...
            //if ($tot_mem_in_room == returnName("room_mem", "rooms", "room_id", $_REQUEST['room_id'])) {
                // $class = " alert alert-danger ";
            ...
            if (1 == 1) {
                ...
                $same_mem_again = returnName("roomr_id", "room_reservation", "room_id", $_REQUEST['room_id'] . " AND  contact_id=" . $_REQUEST['contactid'] . " AND grp_id=" . $_REQUEST['grp_id'] . " AND Pms_Package_ID=" . $_REQUEST['pms_pak_id'] . " AND roomr_startdate='" . $_REQUEST['start_date'] . "' ");
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Room Assigning <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Room Assigning: </b> You can manage your room assigning process here </p>
        </blockquote>
    </div>
</div>
...
<?php } else { ?>
    ...
    <?php echo FillSelected2("rooms", "room_id", "room_number", "room_title", @$_SESSION['room_id']); ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                <thead>
                                    <tr>
                                        ...
                                        <th class="visible-xs visible-sm visible-md visible-lg">Special Instructions</th>
                                        <th class="visible-xs visible-sm visible-md visible-lg">Sizes</th>
                                        <?php
                                        $day_counter = 0;
                                        $day_counter1 = 0;
                                        $date1 = '';
...
AND g.GroupArrivalDate > '2014-05-01'  AND c.ContactFirstName <> ''  AND c.is_completed>=1
".$filterQry."
...
//print($Query);
										...
										//print($Query);
										$trs = mysql_query($Query) or die(mysql_error());
                                        $count = mysql_num_rows($trs);
                                        ...
                                        if (mysql_num_rows($rs) > 0) {
                                            while ($row = mysql_fetch_object($rs)) {
                                                ...
                                                ?>
                                            <tr>
                                                <td class="visible-xs visible-sm visible-md visible-lg"><?php print($row->ContactFirstName); //print($row->ContactID." - ".$row->ContactFirstName); ?></td>
                                                ...
                                                <td class="visible-xs visible-sm visible-md visible-lg"><?php print($row->cont_special_ins); ?></td>
                                                <td class="visible-xs visible-sm visible-md visible-lg"><?php echo 'Jacket Size: ' . $row->jacketsize_name . '<br/> Boot Size: ' . $row->bootsize_name; ?></td>
                                                    <?php
                                                    $day_counter = 0;
                                                    $day_counter1 = 0;
                                                    $date1 = '';
...
//                                                    }
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-windows"></i> Rooms Assignment
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
		...
		$Query = "SELECT p.* FROM packages AS p WHERE p.Arrival_Start_Date > '".$_SESSION['config_date1']."' ORDER BY YEAR(p.Arrival_Start_Date) DESC, p.Arrival_Start_Date ASC";
        //print($Query);
		$counter = 0;
        $limit = $_SESSION['limit_of_rec'];
        $start = $p->findStart($limit);
