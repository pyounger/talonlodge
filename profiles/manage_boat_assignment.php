...
//                mysql_query("UPDATE act_schedule SET act_id = '" . $act_id . "', act_boat_id = '" . $act_boat_id . "', updatedDate = NOW() WHERE asch_id = ".$asch_id." ") or die(mysql_error());
                ...
                mysql_query("INSERT INTO act_schedule (asch_id, cont_id, grp_id, act_id, asch_start_date, asch_end_date, act_boat_id, createdDate, pms_pak_id) VALUES (".$MaxID.", '".$cont_id."', '".$grp_id."', '0', '".$asch_start_date."', '".$asch_start_date."', '".$act_boat_id."', NOW(), '".$_REQUEST['pms_pak_id']."')");
                header("Location: manage_boat_assignment.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=1");
            }
        }
    }    
                            ...
                            <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                ...
                                <tbody>
                                    <?php
										...
										$Query="SELECT p.Pms_Package_ID, p.Package_Name, p.Arrival_Start_Date, p.Arrival_End_Date, p.Package_Max_Days, g.grp_id, g.GroupName, g.GroupArrivalDate, g.GroupDepartureDate, gc.ContactID, c.ContactFirstName, c.ContactLastName, c.cont_image
FROM packages AS p 
                                                        ...
                                                        if ($day_counter != 1) {
                                                            if ($no_of_days == $day_counter) {
                                                                ...
                                                                if ($no_of_days != (1 + $day_counter)) {
                                                                    ...
                                                                    $Query99 = "
                                                                    SELECT DISTINCT 
                                                                    asch.asch_id, asch.act_id, asch.act_boat_id, asch.asch_start_date, asch.asch_end_date, acn.act_name, actb.act_boat_name, ag.ag_name, av.av_name 
                                                                    FROM  
                                                                    ...
                                                                    WHERE 
                                                                    asch.cont_id = ".$row->ContactID." 
                                                                    AND asch.act_id NOT IN (1) 
                                                                    AND asch.asch_start_date LIKE '".$dt->format("Y-m-d")."%'
                                                                    GROUP BY asch.act_id
                                                                    ...
                                                                    if ($count99 > 0) {
                                                                        while ($row99 = mysql_fetch_object($rs99)) {
                                                                            echo (($row99->act_name != '')?'Activity: '.$row99->act_name.'<br/>':'');
                                                                            echo ((date('H:i:s', strtotime($row99->asch_start_date)) != '00:00:00')?'Start: '.date('H:i:s', strtotime($row99->asch_start_date)).'<br/>':'');
                                                                            echo ((date('H:i:s', strtotime($row99->asch_end_date)) != '00:00:00')?'End: '.date('H:i:s', strtotime($row99->asch_end_date)).'<br/>':'');
                                                                            echo (($row99->ag_name != '')?'Guide: '.$row99->ag_name.'<br/>':'');
                                                                            echo (($row99->av_name != '')?'Vendor: '.$row99->av_name.'<br/>':'');
                                                                            echo (($row99->act_boat_name != '')?'Boat: '.$row99->act_boat_name.'<br/>':'');
                                                                    ?>
                                                                    ...
                                                                    <?php
                                                                        ...
                                                                        //echo 'Else <br/>';
                                                                        echo "<input type='hidden' name='act_id[]' value=".'0,'.$dt->format("Y-m-d").',0,0,0'." />";
                                                                    ?>
                                                                        BOAT: <br/> 
