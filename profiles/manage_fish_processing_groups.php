...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Fish Processing Group<?php echo (($_SESSION["UType"]==1)?'s':'');?> <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Fish Processing Groups<?php echo (($_SESSION["UType"]==1)?'s':'');?>: </b> You can manage group information here </p>
        </blockquote>
    </div>
</div>
            ...
            <div class="panel">
                <div class="panel-body">
                    <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                        ...
                        <p>Select guests from the list to setup their individual fish processing preferences.  You can also use the 
                        main group profile management pages to edit guest preferences.  If a guest does not appear in this list, please
                        add a new profile for them on the group profiles page.</p>
...
*/
    ...
    ?>
                        ...
                        $Query2 = "SELECT gc.grp_id, c.ContactID,c.cont_id, c.ContactFirstName, c.ContactLastName
                                                ,c.is_completed, cp.lastUpdated, fp.id as fish_id
                                                ...
                                                LEFT JOIN fish_processing AS fp ON fp.profile_id = c.cont_id
                                                WHERE g.grp_id = '".$_REQUEST["grp_id"]."' 
                                                GROUP BY c.cont_id
                        ...
                        ?><ul>
                           <?php foreach ($contacts as $contact) { 
                            echo "<li>";
                                                ...
                                                //$url = "/profiles/manage_profile.php?action=2&cont_id=".$contact["cont_id"]."&contactid=".$contact["ContactID"]."&grp_id=".$package["grp_id"]."#fish";
                                                $url = "/profiles/manage_profile.php?action=2&cont_id=".$contact["cont_id"]."&contactid=".$contact["ContactID"]."&grp_id=".$contact["grp_id"]."#fish";
                                                ...
                                                if ($contact["fish_id"]>0) {
                                                        $style = "scheduled";
                                                        $fish_note = "Fish Preferences Exist";
                                                    } else {
                                                        $style = "";
                                                        $fish_note = "No Fish Preferences Found!";
                                                    }

                                                echo "<a href='$url' target='_blank'><span class=\"".$style."\">";
                                                echo $contact["ContactFirstName"] . " " . $contact["ContactLastName"] . " - " . $fish_note;
                                                echo "</span></a>";
 } ?>
                        </ul>
