                    ...
                    <div class="row">
                        <div class="">
                            <ul class="nav faq-list">
                                ...
                                WHERE g.grp_id > 0 AND p.Arrival_Start_Date > ? AND c.is_completed>=1
                                ...
                                $Query3 = "SELECT cp.conp_id, cp.cont_id, ca.act_id, a.act_name, 
                                                    ...
                                                    asch.asch_id, asch.cont_id AS aschcont_id, asch.asch_start_date, 
                                                    asch.asch_end_date, asch.asch_duration, asch.asch_email_counter, 
                                                    ag.ag_name, ad.ad_name, av.av_name, abn.act_boat_name, 
                                                    thr.ath_id, thr.ath_name, thrt.atht_id, thrt.atht_name 
                                                    FROM  
                                            contact_profiles AS cp 
                                            LEFT JOIN contacts AS c ON c.cont_id = cp.cont_id
                                            LEFT JOIN group_contacts AS gc ON gc.ContactID = c.ContactID
                                            LEFT JOIN contact_activities AS ca ON cp.conp_id=ca.conp_id                                             
                                            LEFT JOIN activities AS a ON ca.act_id = a.act_id
                                            LEFT JOIN groups AS g ON g.grp_id = gc.grp_id                                            
                                            LEFT JOIN act_schedule AS asch ON asch.grp_id = g.grp_id AND asch.cont_id = cp.cont_id AND asch.act_id=a.act_id                                            
                                            LEFT JOIN activity_guides AS ag ON asch.ag_id=ag.ag_id 
                                            LEFT JOIN activity_destination AS ad ON asch.ad_id=ad.ad_id 
                                            LEFT JOIN activity_vendors AS av ON asch.av_id=av.av_id 
                                            LEFT JOIN act_therapist AS thr ON asch.ath_id=thr.ath_id 
                                            LEFT JOIN act_th_type AS thrt ON asch.atht_id=thrt.atht_id 
                                            LEFT JOIN act_boats AS abn ON asch.act_boat_id=abn.act_boat_id 
                                            INNER JOIN packages AS p ON g.Pms_Package_ID = p.Pms_Package_ID 
                                            WHERE
                                            g.grp_id > 0 AND p.Arrival_Start_Date > ? AND c.is_completed>=1
                                ...
                                foreach ($packages as $package) {
                                    ...
                                    foreach ($contacts as $contact) {
                                        ...
                                        if ($contact["grp_id"] == $package["grp_id"]) {
                                            ...
                                            echo $contact["ContactFirstName"] . " " . $contact["ContactLastName"] . "<br/>";
                                            foreach ($activities as $activity) {
                                                echo "<ul>";
                                                if ($contact["cont_id"] == $activity["cont_id"]) {
                                                    echo "<li>";
                                                    //print_r($activity); //uncomment to view array.
                                                    if ($activity["asch_id"] > 0) {
                                                        echo "<span style='background-color:#00b3ee;'>" . $activity["act_name"] . " - " . date("m/d/Y", strtotime($activity["asch_start_date"])) . ",<span><br/>";
                                                    } else {
