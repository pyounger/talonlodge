...
if (isset($_REQUEST['grp_id'])) {
    ...
    if (!isset($_SESSION['group_id'])) {
        ...
        $group_id1 = returnName("grp_id", "contacts", "ContactID", $_REQUEST['contactid']);
        if ($group_id1 > 0) {
            $_SESSION['group_id'] = $group_id1;
        }
    }
}
//print("<br><br>GROUP ID: ".$_SESSION['group_id']."<br><br><br>");
...
if (isset($_REQUEST['send'])) {
    $rs = mysql_query("SELECT u.*, c.cont_email FROM users AS u LEFT OUTER JOIN contacts AS c ON c.ContactID=u.cont_id WHERE u.cont_id=" . $_REQUEST['contactid']);
    if (mysql_num_rows($rs) > 0) {
        $row = mysql_fetch_object($rs);
        mysql_query("UPDATE contacts SET is_email = (is_email + 1) WHERE ContactID = " . $_REQUEST['contactid'] . "");
        $strQryGrp = "";
        if (isset($_REQUEST['grp_id'])) {
            $strQryGrp = "&grp_id=" . $_REQUEST['grp_id'];
        }
        $linkStrQry = $_REQUEST['contactid'] . $strQryGrp;
        //emailToCustomer(dbStr($row->cont_email), dbStr($row->user_pasphrase), dbStr($row->user_display_name), dbStr($_REQUEST['contactid']), dbStr($row->user_name));
        emailToCustomer(dbStr($row->cont_email), dbStr($row->user_pasphrase), dbStr($row->user_display_name), dbStr($linkStrQry), dbStr($row->user_name));
        ...
        if (mysql_num_rows($rs) > 0) {
            ...
            if ($row->Email != '') {
                if ($_REQUEST['send'] == 1) {
                    ...
                    $fName = explode(" ", $row->ContactFirstName);
                    $userName = $fName[0] . "_" . $row->ContactID;
                    mysql_query("INSERT INTO users (user_id, user_name, user_pass, user_display_name, utype_id, cust_id, cont_id, createdDate, user_pasphrase) VALUES (" . $MaxID . ", '" . dbStr($userName) . "', '" . md5($random_password) . "', '" . $contact_name . "', '3', " . $row->cust_id . ", " . $row->ContactID . ", NOW(), '" . $random_password . "')");

                    $rs1 = mysql_query("SELECT * FROM users WHERE cont_id=" . $_REQUEST['contactid']);
                    if (mysql_num_rows($rs1) > 0) {
                        $row1 = mysql_fetch_object($rs1);
                        mysql_query("UPDATE contacts SET is_email = (is_email + 1) WHERE ContactID = " . $_REQUEST['contactid'] . "");
                        emailToCustomer(dbStr($row1->user_name), dbStr($row1->user_pasphrase), dbStr($row1->user_display_name), dbStr($_REQUEST['contactid']), $userName);
                    }
                    $strMSG = " Login Info sent ";
                    $class = "alert alert-success";
                } else {
                    ...
                    $fName = explode(" ", $row->ContactFirstName);
                    $userName = $fName[0] . "_" . $row->ContactID;
                    mysql_query("INSERT INTO users (user_id, user_name, user_pass, user_display_name, utype_id, cust_id, cont_id, createdDate, user_pasphrase) VALUES (" . $MaxID . ", '" . dbStr($userName) . "', '" . md5($random_password) . "', '" . $contact_name . "', '2', " . $row->cust_id . ", " . $row->ContactID . ", NOW(), '" . $random_password . "')") or die(mysql_error());
                    $rs1 = mysql_query("SELECT * FROM users WHERE cont_id=" . $_REQUEST['contactid']);
                    if (mysql_num_rows($rs1) > 0) {
                        $row1 = mysql_fetch_object($rs1);
                        mysql_query("UPDATE contacts SET is_email = (is_email + 1) WHERE ContactID = " . $_REQUEST['contactid'] . "");
...
if (isset($_REQUEST['btnInvite'])) {
    ...
    if (isset($_REQUEST['chkContactID'])) {
        for ($i = 0; $i < count($_REQUEST['chkContactID']); $i++) {
            $chk = chkExist("gcont_id", "group_contacts", "WHERE grp_id=" . $_REQUEST['grp_id'] . " AND ContactID=" . $_REQUEST['chkContactID'][$i]);
            if ($chk == 0) {
                $gcont_id = getMaximum("group_contacts", "gcont_id");
                mysql_query("INSERT INTO group_contacts (gcont_id, grp_id, Pms_Booking_ID, ContactID, GroupArrivalDate, createdDate) VALUES(" . $gcont_id . ", '" . $_REQUEST['grp_id'] . "', '" . $Pms_Booking_ID . "', " . $_REQUEST['chkContactID'][$i] . ", '" . $grp_arrival_date . "', NOW())");
                $tinfo_id = getMaximum("travel_info", "tinfo_id");
                $contid = returnName("cont_id", "contacts", "ContactID", $_REQUEST['chkContactID'][$i]);
                mysql_query("INSERT INTO travel_info (tinfo_id, cont_id, grp_id, Pms_Booking_ID, CreatedDate, ContactID, Mag_ContactID) VALUES (" . dbStr($tinfo_id) . ", " . dbStr($contid) . ", '" . dbStr($_REQUEST['grp_id']) . "', '" . dbStr(@$Pms_Booking_ID) . "', NOW(), '" . $_REQUEST['chkContactID'][$i] . "', '0')");
            }
        }
        // ADD LOG ENTRY
        //$MaxID = getMaximum("logs", "log_id");
        //mysql_query("INSERT INTO logs (log_id, cont_id, ContactID, user_id, log_datetime, log_type) VALUES ('".$MaxID."', '".$contid."', '".$ContactID."', '".$_SESSION["contact_id"]."', NOW(), '1')");
        $class = "alert alert-success";
...
if (isset($_REQUEST['action'])) {
    if (isset($_REQUEST['btnAdd'])) {
        ...
        $fName = explode(" ", $_REQUEST['cont_fname']);
        $userName = $fName[0] . "_" . $ContactID;
        ...
        mysql_query("INSERT INTO users(user_id, user_name, user_pass, user_display_name, utype_id, cust_id, cont_id, createdDate, user_pasphrase) VALUES(" . $userid . ",'" . dbStr($userName) . "', '" . md5($random_number) . "','" . $contact_name . "', '3', '" . $ContactID . "', '" . $ContactID . "', NOW(), '" . $random_number . "')");
        ...
        if ((isset($_REQUEST['grp_id'])) && ($_REQUEST['grp_id'] != '') && ($_REQUEST['grp_id'] != 0)) {
            mysql_query("INSERT INTO group_contacts (gcont_id, grp_id, Pms_Booking_ID, ContactID, GroupArrivalDate, createdDate) VALUES(" . $gcont_id . ", '" . $_REQUEST['grp_id'] . "', '" . $Pms_Booking_ID . "', " . $ContactID . ", '" . $grp_arrival_date . "', NOW())");
        ...
        $arr_hotel_id = '';
        $arr_con_private_jet = '';
        if (($_REQUEST['arr_flight_id'] == 1) || ($_REQUEST['arr_flight_id'] > 4)) {
        ...
        $dep_hotel_id = '';
        $dep_con_private_jet = '';
        if (($_REQUEST['dep_flight_id'] == 1) || ($_REQUEST['dep_flight_id'] > 4)) {
        ...
        if ((isset($_REQUEST['grp_id'])) && ($_REQUEST['grp_id'] != '') && ($_REQUEST['grp_id'] != 0)) {
            ...
            $tinfo_id = chkExist("tinfo_id", "travel_info", " WHERE cont_id = '" . $_REQUEST['cont_id'] . "' AND grp_id = '" . $_REQUEST['grp_id'] . "' AND ContactID = '" . $_REQUEST['contactid'] . "' ");
            if ($tinfo_id > 0) {
                $udtQuery = "UPDATE travel_info SET 
					LastUpdated = NOW(),
					arrival_flight_data = '" . calendarDateConver4($_REQUEST['arrival_flight_data']) . "',
					departure_flight_date = '" . calendarDateConver4($_REQUEST['departure_flight_date']) . "',
					con_flight_comments = '" . dbStr($_REQUEST['con_flight_comments']) . "',
					...
					is_completed = " . dbStr($is_completed) . "
				WHERE
				cont_id = '" . $_REQUEST['cont_id'] . "' AND grp_id = '" . $_REQUEST['grp_id'] . "' AND ContactID = '" . $_REQUEST['contactid'] . "' ";
                ...
                mysql_query("
					...
					(
                                            " . dbStr($tinfo_id) . ",
                                            '" . dbStr($_REQUEST['cont_id']) . "',
                                            '" . dbStr($_REQUEST['grp_id']) . "',
                                            '" . dbStr(@$Pms_Booking_ID) . "',
                                            NOW(),
                                            '" . calendarDateConver4($_REQUEST['arrival_flight_data']) . "',
                                            '" . calendarDateConver4($_REQUEST['departure_flight_date']) . "',
                                            '" . dbStr($_REQUEST['contactid']) . "',
                                            '" . dbStr(@$_REQUEST['con_flight_comments']) . "',
                                            '" . dbStr(@$_REQUEST['arr_flight_id']) . "',
                                            '" . dbStr(@$arr_flightn_id) . "',
                                            '" . dbStr(@$arr_flightt_id) . "',
                                            '" . dbStr(@$arr_hotel_id) . "',
                                            '" . dbStr(@$arr_con_private_jet) . "',
                                            '" . dbStr(@$_REQUEST['dep_flight_id']) . "',
                                            '" . dbStr(@$dep_flightn_id) . "',
                                            '" . dbStr(@$dep_flightt_id) . "',
                                            '" . dbStr(@$dep_hotel_id) . "',
                                            '" . dbStr(@$dep_con_private_jet) . "',
                                            '" . dbStr($is_completed) . "',
                                            '0'
					)
				");
        ...
        mysql_query("UPDATE users SET user_display_name='" . $contact_name . "', lastUpdated=NOW() WHERE cont_id = '" . $_REQUEST['contactid'] . "' ");
        ...
        /*
          echo '<pre><div style="margin-left:100px; margin-top:100px; font-size:20px;">';
          print_r ( $_REQUEST );
          print_r ( $_FILES );
          echo '</div></pre>';
         */
        ...
        /**
        * Start Fish Processing Editing.
        * Stella International, Inc.
        */
        
        if ($_REQUEST["fish_id"]>0) {
            //Update
            $fish_update = " update fish_processing set "
                    . "      delivery_type_id = ?, shipping_arrival_date = ?, shipping_address1 = ?, shipping_address2 = ?,"
                    . "      shipping_city = ?, shipping_state_id = ?, shipping_zip = ?, shipping_contact_phone = ?, shipping_contact_name = ?,"
                    . "      shipping_tracking_number = ?, special_instructions = ?, smoked = ?, smoking_instructions = ? where id = ? ";
            $fish_shipping_arrival_date = ($_REQUEST["fish_shipping_arrival_date"]<>"")?date("Y-m-d",strtotime($_REQUEST["fish_shipping_arrival_date"])):"";
            $params = array(
                    $_REQUEST["fish_delivery_type_id"],
                    $fish_shipping_arrival_date,
                    $_REQUEST["fish_shipping_address1"],
                    $_REQUEST["fish_shipping_address2"],
                    $_REQUEST["fish_shipping_city"],
                    $_REQUEST["fish_shipping_state_id"],
                    $_REQUEST["fish_shipping_zip"],
                    
                    $_REQUEST["fish_shipping_contact_phone"],
                $_REQUEST["fish_shipping_contact_name"],
				$_REQUEST["fish_shipping_tracking_number"],
                    $_REQUEST["fish_special_instructions"],
                    $_REQUEST["fish_smoked"],
                    $_REQUEST["fish_smoking_instructions"],
                    $_REQUEST["fish_id"]
            );
            $fish_result = UpdateData($fish_update,$params);
            
        } else {
            //Insert
            $fish_insert = " insert into fish_processing "
                    . "      (id, group_id, profile_id, delivery_type_id, shipping_arrival_date, shipping_address1, shipping_address2,"
                    . "      shipping_city, shipping_state_id, shipping_zip, shipping_contact_phone, shipping_contact_name, shipping_tracking_number,"
                    . "      special_instructions, smoked, smoking_instructions) values ('',?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) ";
            $fish_shipping_arrival_date = ($_REQUEST["fish_shipping_arrival_date"]<>"")?date("Y-m-d",strtotime($_REQUEST["fish_shipping_arrival_date"])):"";
            $params = array(
                    $_REQUEST["grp_id"],
                    $_REQUEST["cont_id"],
            ...
            $fish_result = InsertData($fish_insert,$params);
            
        }
        
        //echo $fish_result; exit;
        
        /**
        * //END Fish Processing Editing.
        * Stella International, Inc.
        */
        ...
        // UPDATE LOG ENTRY
        $MaxID = getMaximum("logs", "log_id");
        mysql_query("INSERT INTO logs (log_id, cont_id, ContactID, user_id, log_datetime, log_type) VALUES ('" . $MaxID . "', '" . $_REQUEST['cont_id'] . "', '" . $_REQUEST['contactid'] . "', '" . $_SESSION["contact_id"] . "', NOW(), '0')");
		...
		c.cont_city, c.cont_state, c.cont_zip, c.countries_id, c.cont_phone1, c.cont_phone2, c.cont_image, c.gen_id, cp.*, t.* FROM contacts As c LEFT OUTER JOIN contact_profiles AS cp ON c.ContactID=cp.cont_id LEFT OUTER JOIN travel_info AS t ON c.cont_id = t.cont_id AND c.ContactID = t.ContactID AND c.grp_id = t.grp_id WHERE c.ContactID = " . $_REQUEST['contactid'] . " LIMIT 1");
        ...
        if (mysql_num_rows($rsM) > 0) {
            ...
            $formHead = "Step 1 - Review & Update Profile Information: ";
        }
    } else {
        $cont_id = 0;
        $cont_fname = "";
...
<span class="input-icon fui-search"></span> </div>-->
        <h3 class="page-header"> Manage Guest Profile <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Guest Profile: </b> You can manage your guest profiles here </p>
        </blockquote>
    </div>
</div>
<?php if (isset($_REQUEST['action'])) { ?>
        ...
        <div class="col-md-12">
            <div class="panel panel-cascade">
                <div class="panel-heading">
                                            ...
                                            <div class="col-mol-md-offset-2">
                                                ...
                                                <form name="frm21" id="frm21" method="post" class="form-horizontal cascde-forms" action="<?php print($_SERVER['PHP_SELF']); ?>">
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-lg-10 col-md-9"> <?php echo @$sl_group_name2; ?> </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label class="col-lg-2 col-md-3 control-label">Arrival Date:</label>
                                                        <div class="col-lg-10 col-md-9"> <?php echo @$sl_ard_date; ?> </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label class="col-lg-2 col-md-3 control-label">Departure Date:</label>
                                                        <div class="col-lg-10 col-md-9"> <?php echo @$sl_dpd_date; ?> </div>
                                                    </div>
                                                    <div class="form-group">
                                                        <label class="col-lg-2 col-md-3 control-label">Package:</label>
                                                        <div class="col-lg-10 col-md-9"> <?php echo @$sl_pk; ?> </div>
                                                    </div>
                                                </form>
                                                <?php //} ?>
                                            </div>
                    ...
                    <h3 class="panel-title text-primary">
                        <?php print($formHead); ?>
                        <?php if (isset($_REQUEST['grp_id']) && $_REQUEST['grp_id'] > 0) { ?>
                            <?php if (!isset($_REQUEST['invite_guest'])) { ?>
                                <?php if (isset($_REQUEST['cont_id'])) { ?>
                                    <div style="float:right;">
                                        <div class="tooltips"><a href="<?php print($_SERVER['PHP_SELF'] . "?show=1&cont_id=" . $_REQUEST['cont_id'] . "&contactid=" . $_REQUEST['contactid'] . "&grp_id=" . $_REQUEST['grp_id']); ?>" data-original-title="Travel Info" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-plane"></i>&nbsp;&nbsp; Flight Info</a></div>
                                    </div>
                                <?php } ?>
                            <?php } ?>
                        <?php } ?>
                ...
                <div class="panel-body panel-border">
                    ...
                    <form name="frm" id="frm" method="post" class="form-horizontal" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>"  enctype="multipart/form-data"  role="form">
                        ...
                        <?php } else { ?>
                            <input type="hidden" name="grp_id" id="grp_id" value="<?php print(@$_SESSION['group_id']); ?>">
                        <?php } ?>
                        <?php if (isset($_REQUEST['invite_guest']) && $_REQUEST['invite_guest'] == 2) { ?>
                            <table class="table users-table table-condensed table-hover table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="visible-xs visible-sm visible-md visible-lg">Invited</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $invitedIDs = array();
                                    $r = mysql_query("SELECT ContactID FROM group_contacts WHERE grp_id=" . $_REQUEST['grp_id']);
                                    while ($rw = mysql_fetch_object($r)) {
                                        $invitedIDs[] = $rw->ContactID;
                                    }
                                    //print_r($invitedIDs);
                                    $grpLeaderID = returnName("Contact_ID", "groups", "grp_id", $_REQUEST['grp_id']);
                                    $Query = "SELECT gc.*, c.cont_fname, c.cont_lname, c.cont_email, c.cont_phone1 FROM group_contacts AS gc 
						LEFT OUTER JOIN contacts AS c ON c.ContactID =gc.ContactID
						WHERE gc.grp_id IN (SELECT g.grp_id FROM groups AS g WHERE g.GroupArrivalDate > '2014-01-01' AND g.Contact_ID=" . $grpLeaderID . ") AND gc.ContactID <> '" . $grpLeaderID . "'
						GROUP BY gc.ContactID";
                                    ...
                                    if (mysql_num_rows($rs) > 0) {
                                        while ($row = mysql_fetch_object($rs)) {
                                            ...
                                            ?>
                                            <tr>
                                                <td class="visible-xs visible-sm visible-md visible-lg"><input type="checkbox" name="chkContactID[]" value="<?php print($row->ContactID); ?>" /></td>
                                                ...
                                                <td class="visible-md visible-lg"><?php print($row->cont_email); ?> </td>
                                                <td class="visible-md visible-lg"><?php print($row->cont_phone1); ?> </td>
                                                <td class="visible-xs visible-sm visible-md visible-lg">
                                                    <?php if (in_array($row->ContactID, $invitedIDs)) { ?>
                            ...
                            <?php if ($counter > 0 && $count > 20) { ?>
                                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                    <tr>
                                        ...
                                        <td align="right">
                                            <?php
                                            $next_prev = $p->nextPrev($_GET['page'], $pages, '&action=1&invite_guest=2&grp_id=' . $_REQUEST['grp_id']);
                                            print($next_prev);
                                            ?>
                                        </td>
                                    </tr>
                            ...
                            <div class="form-group" style="padding-left:20px;">
                                <button type="submit" name="btnInvite" class="btn btn-primary btn-animate-demo">Submit</button>&nbsp;&nbsp;&nbsp;
                                <button type="button" name="btnCancel" class="btn btn-default btn-animate-demo" onclick="javascript: window.location = '<?php print($_SERVER['PHP_SELF'] . "?grp_id=" . $_REQUEST['grp_id']); ?>';">Back</button>
                            </div>
                        <?php } else { ?>
                            <div class="form-group">
                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact First Name</label>
                                <div class="col-lg-10 col-md-9"><input type="text" class="form-control form-cascade-control input_wid70 required" placeholder="Contact First Name..." value="<?php print($cont_fname); ?>" id="cont_fname" name="cont_fname"></div>
                            </div>
                            <div class="form-group">
                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Last Name</label>
                                <div class="col-lg-10 col-md-9"><input type="text" class="form-control form-cascade-control input_wid70 required" placeholder="Contact Last Name..." value="<?php print($cont_lname); ?>" id="cont_lname" name="cont_lname"></div>
                            </div>
                            <div class="form-group">
                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Email</label>
                                <div class="col-lg-10 col-md-9"><input type="text" class="form-control form-cascade-control input_wid70 required" placeholder="Contact Email..." value="<?php print($cont_emial); ?>" id="cont_email" name="cont_email"></div>
                            </div>
                            <div class="form-group">
                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Phone </label>
                                <div class="col-lg-10 col-md-9"><input type="text" class="form-control form-cascade-control input_wid70" placeholder="Phone Number..." value="<?php print($cont_phone1); ?>" id="phone-input1" name="cont_phone1"></div>
                            </div>
                            ...
                            <div style="display: <?php echo $guest_display; ?>">
                                ...
                                <div class="form-group" id="with_states">
                                    ...
                                    <div class="col-lg-10 col-md-9">
                                        <select data-placeholder="Choose a State..." name="cont_state1" class="chosen-" tabindex="2" style=" width:260px; ">
                                            ...
                                            <?php
                                            if ($cont_state != '' && strlen($cont_state) > 2) {
                                                $cont_state = getStateAbbreviation($cont_state);
                                            }
                                            ?>
                                            <?php echo FillSelected("states", "abbrev", "name", $cont_state); ?>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <?php if ((isset($_REQUEST['grp_id'])) && ($_REQUEST['grp_id'] != '') && ($_REQUEST['grp_id'] != 0) && isset($_REQUEST['cont_id'])) { ?>
                                    <hr style="padding:1px; background-color:#999">
                                    <h3> Travel Information </h3>
                                    <?php
                                    $tInfoQry = "SELECT * FROM travel_info WHERE cont_id = '" . $_REQUEST['cont_id'] . "' AND grp_id = '" . $_REQUEST['grp_id'] . "' AND ContactID = '" . $_REQUEST['contactid'] . "' ";
                                    //print($tInfoQry);
                                    $tinfo = mysql_query($tInfoQry);

                                    if (mysql_num_rows($tinfo) > 0) {
                                        $tir = mysql_fetch_object($tinfo);
                                        //var_dump($tir);
                                        $arrival_flight_data = calendarDateConver2($tir->arrival_flight_data);
                                        $departure_flight_date = calendarDateConver2($tir->departure_flight_date);
                                        //$tir->con_flight_comments;
                                        $arr_flight_id = $tir->arr_flight_id;
                                        $arr_flightn_id = $tir->arr_flightn_id;
                                        $arr_flightt_id = $tir->arr_flightt_id;
                                        $arr_flightt_id2 = $tir->arr_flightt_id;
                                        $arr_hotel_id = $tir->arr_hotel_id;
                                        $arr_con_private_jet = $tir->arr_con_private_jet;
                                        $dep_flight_id = $tir->dep_flight_id;
                                        $dep_flightn_id = $tir->dep_flightn_id;
                                        $dep_flightt_id = $tir->dep_flightt_id;
                                        $dep_hotel_id = $tir->dep_hotel_id;
                                        $dep_con_private_jet = $tir->dep_con_private_jet;
                                        $con_flight_comments = $tir->con_flight_comments;
                                        $is_completed = $tir->is_completed;
                                    }
                                    $fltr = "";
                                    if (isset($arr_flight_id)) {
                                        $fltr = " AND flight_id=" . $arr_flight_id;
                                    }
                                    $fltrPlease = "";
                                    if (isset($arr_flight_id)) {
                                        $fltrPlease = " AND flight_id=" . $arr_flight_id;
                                    }
                                    ?>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group" id="arrival_flight1">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            <select data-placeholder="Arrival Flight Number..." name="arr_flightn_id" id="arr_flightn_id" class="" style="width:260px;" tabindex="2" onChange="javascript:get_arr_time(this.value);">
                                                ...
                                                <?php FillSelected(" flight_no WHERE flightn_status=1 " . $fltr, "flightn_id", "flightn_name", @$arr_flightn_id); ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <script>
                                        ...
                                        function fillFlightNumArr(flight_id) {
                                            var url = "services.php?fltNum=1&flight_id=" + flight_id + "&flightn_status=1";
                                            $.ajax({
                                                type: "POST",
                                                url: url,
                                                data: null,
                                    ...
                                    <div class="form-group" id="arrival_flight3">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            <select data-placeholder="Arrival Flight Time..." name="arr_flightt_id2" id="arr_flightt_id" class="" style="width:260px;" tabindex="2">
                                                <option value=""></option>
                                                <?php FillSelected(" flight_time WHERE flightt_status=3 " . $fltrPlease, "flightt_id", "flightt_name", @$arr_flightt_id); ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            ...
                                            <input type="text" class="form-control form-cascade-control input_wid70 datetimepicker" placeholder="Arrival Flight Date..." value="<?php
                                            $ar_dat1 = (($arrival_flight_data == '00/00/0000') ? '' : $arrival_flight_data);
                                            $ar_dat2 = @calendarDateConver2(@returnName(" g.GroupArrivalDate ", " groups AS g ", " g.grp_id ", $_SESSION['group_id']));
                                            echo (($ar_dat1 != '') ? $ar_dat1 : $ar_dat2);
                                            ?>" id="arrival_flight_data" name="arrival_flight_data">
                                        </div>
                                    </div>
                                    <div class="form-group">
            ...
            <?php FillSelected(" flight_no WHERE flightn_status=2 ", "flightn_id", "flightn_name", @$dep_flightn_id); ?>
                                    ...
                                    <script>
                                        ...
                                        function fillFlightNumDep(flight_id) {
                                            var url = "services.php?fltNum=1&flight_id=" + flight_id + "&flightn_status=2";
                                            $.ajax({
                                                type: "POST",
                                                url: url,
                                                data: null,
            ...
            <?php FillSelected(" hotel_info ", "hotel_id", "hotel_name", @$dep_hotel_id); ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            ...
                                            <input type="text" class="form-control form-cascade-control input_wid70  datetimepicker" placeholder="Departure Flight Date..." value="<?php
                                $dr_dat1 = (($departure_flight_date == '00/00/0000') ? '' : $departure_flight_date);
                                $dr_dat2 = @calendarDateConver2(@returnName(" g.GroupDepartureDate ", " groups AS g ", " g.grp_id ", $_SESSION['group_id']));
                                echo (($dr_dat1 != '') ? $dr_dat1 : $dr_dat2);
                                ?>" id="departure_flight_date" name="departure_flight_date">
                                        </div>
                                    </div>
                                    <div class="form-group">
            ...
            ?>
                                                ...
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            ...
                                                            &nbsp;  &nbsp; <span style="float: left;"> &nbsp;  &nbsp; Activity Info:</span> <a href="javascript:void(0);" data-original-title="<?php echo $row->act_details; ?>" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-info-circle"></i></a> &nbsp; &nbsp; <span style="float: left;"> &nbsp;  Price:</span> <a href="javascript:void(0);" data-original-title="<?php echo $row->act_price; ?>" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-usd"></i></a> &nbsp; &nbsp; <span style="float: left;"> &nbsp;  Website:</span> <a href="<?php echo $row->act_link; ?>" target="_blank" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-link"></i></a> &nbsp; 
                                    ...
                                    ?>
                                    <hr style="padding:1px; background-color:#999"><a name="fish"></a>
                                    <h3> Fish Processing Options </h3>
                                    <?php
                                    $counter = 0;
                                    if (isset($_REQUEST['cont_id'])) {
                                        $qry_fish_processing = "SELECT * from fish_processing t1 where profile_id= ?  ";
                                        $params = array($_REQUEST['cont_id']);
                                        $row_fish = SelectRecord($qry_fish_processing,$params);
                                    } 
                                    //var_dump($row_fish); exit;
                                    if (!($row_fish)) {
                                        $row_fish = array(
                                            "id" => -1,
                                            "delivery_type_id" => 0,
                                            "shipping_arrival_date" => "0000-00-00",
                                            "shipping_address1" => "",
                                            "shipping_address2" => "",
                                            "shipping_city" => "",
                                            "shipping_state_id" => "0",
                                            ...
                                            "smoked" => "",
                                            "smoking_instructions" => ""
                                        );
                                    }
                                    ?>
                                            <div class="form-group">
                                                <input type="hidden" name="fish_id" value="<?php echo $row_fish["id"]; ?>">
                                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Delivery Type</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <select placeholder="Select..." class="form-control input_wid70" name="fish_delivery_type_id" id="fish_delivery_type_id" >
                                            <option value="0"></option>
                                            <?php
                                            if ($row_fish["delivery_type_id"] == '') {
                                                $row_fish["delivery_type_id"] = 0;
                                            }
                                            ?>
                                            <?php echo FillSelected("fish_delivery_types", "id", "delivery_type", $row_fish["delivery_type_id"]); ?>
                                        </select> 
                                                        </div>
                                                    </div>
                                                </div>
                                                <div id="row_shipping_instructions" style="display:none;" > 
                                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Shipping Arrival Date</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input name="fish_shipping_arrival_date" type="text" class="form-control input_wid70 datetimepicker" value="<?php echo ($row_fish["shipping_arrival_date"]=="0000-00-00"||$row_fish["shipping_arrival_date"]=="1969-12-31")?"":date("m/d/Y",strtotime($row_fish["shipping_arrival_date"])); ?>" placeholder="mm/dd/yyyy" />
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                <label for="site_login" class="col-lg-2 col-md-3 control-label">Ship To Contact</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input name="fish_shipping_contact_name" class="form-control input_wid70"  type="text" value="<?php echo $row_fish["shipping_contact_name"]; ?>" placeholder="" />
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                 <label for="site_login" class="col-lg-2 col-md-3 control-label">Ship To Phone</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input name="fish_shipping_contact_phone" type="text" class="form-control input_wid70" value="<?php echo $row_fish["shipping_contact_phone"]; ?>" placeholder="(___) - ___ - ____" />
                                                        </div>
                                                    </div>
                                                </div>
                                                
                                                 <label for="site_login" class="col-lg-2 col-md-3 control-label">Ship To Address</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input name="fish_shipping_address1" type="text" class="form-control input_wid70" value="<?php echo $row_fish["shipping_address1"]; ?>" placeholder="Address 1" /> <br>
                                                            <input name="fish_shipping_address2" type="text" class="form-control input_wid70" value="<?php echo $row_fish["shipping_address2"]; ?>" placeholder="Address 2" /> <br>
                                                            <input name="fish_shipping_city" type="text" class="form-control" style="width:30%" value="<?php echo $row_fish["shipping_city"]; ?>" placeholder="City" /><br>
                                                            <select placeholder="State" name="fish_shipping_state_id" style="width:20%" class="form-control " >
                                            <option value="0">State</option>
                                            <?php
                                            if ($row_fish["shipping_state_id"] == '') {
                                                $row_fish["shipping_state_id"] = 0;
                                            }
                                            ?>
                                            <?php echo FillSelected("states", "id", "abbrev", $row_fish["shipping_state_id"]); ?>
                                        </select>
                                                            <br><input name="fish_shipping_zip" type="text" style="width:20%"  class="form-control " value="<?php echo $row_fish["shipping_zip"]; ?>" placeholder="ZIP" />
                                                        </div>
                                                    </div>
                                                </div>
												
												 <label for="site_login" class="col-lg-2 col-md-3 control-label">Shipping Tracking Number</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input name="fish_shipping_tracking_number" type="text" class="form-control input_wid70" value="<?php echo $row_fish["shipping_tracking_number"]; ?>" placeholder="Shipping Tracking Number" />
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <textarea type="text" class="form-control form-cascade-control input_wid70 " placeholder="comment.."  id="fish_special_instructions" name="fish_special_instructions"><?php print($row_fish["special_instructions"]); ?></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                                   
                                                    <label for="site_login" class="col-lg-2 col-md-3 control-label">Smoke the Fish?</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <input type="radio" name="fish_smoked" class="fish_smoked" id="fish_smoked_1"  value="1" <?php echo ($row_fish["smoked"]=="1")?"checked":""; ?>> Yes
                                                            <input type="radio" name="fish_smoked"  class="fish_smoked" id="fish_smoked_0"  value="0" <?php echo ($row_fish["smoked"]=="0")?"checked":""; ?>> No
                                                        </div>
                                                    </div>
                                                </div>
                                                   <div id="row_fish_smoking_instructions" style="display:none;" > 
                                                     <label for="site_login" class="col-lg-2 col-md-3 control-label">Smoking Instructions</label>
                                                <div class="col-lg-10 col-md-9">
                                                    <div class="tooltips">
                                                        <div class="list-group">
                                                            <textarea type="text" class="form-control form-cascade-control input_wid70 " placeholder="comment.."  id="fish_smoking_instructions" name="fish_smoking_instructions"><?php print($row_fish["smoking_instructions"]); ?></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                                   </div>
            ...
            <div class="panel panel-cascade">
                ...
                <div class="panel-body">
                    <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact First Name</label>
                            <div class="col-lg-10 col-md-9 det-display" style="text-transform:capitalize;"> <?php print($cont_fname); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Last Name</label>
                            <div class="col-lg-10 col-md-9 det-display" style="text-transform:capitalize;"> <?php print($cont_lname); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Email</label>
                            <div class="col-lg-10 col-md- det-display"> <?php print($cont_emial); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Contact Phone </label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_phone1); ?> 
    ...
    <?php print($countries_name); //FillSelected("countries", "countries_id", "countries_name", $countries_id);    ?> 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_address1); ?> 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_address2); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">City</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_city); ?> 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_state); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Zip Code</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_zip); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Emergency Contact Name</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($emg_contact_name); ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Emergency Contact Phone</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($cont_phone2); ?> 
    ...
    <?php print($gen_name); //FillSelected("gender", "gen_id", "gen_name", $gen_id);    ?> 
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Age</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($conp_age); ?> 
                        ...
                        <?php
                        $counter_qn = 0;
                        $Query = "SELECT q.*, cpd.question_id AS cpdquestion_id, cpd.istrue, cpd.cpd_answer FROM questions AS q, contact_profile_details AS cpd WHERE q.question_id=cpd.question_id AND cpd.cont_id=" . $_REQUEST['contactid'] . " AND cpd.cpd_answer!='' AND q.status_id=1 AND q.question_id<3 ORDER BY q.question_id";
    ...
    <?php if (!isset($_REQUEST['grp_id'])) { ?>
                            ...
                            <div class="form-group" style="padding:2px 15px;">
                                <table class="table users-table table-condensed table-hover table-striped display dataTable" id="example1" >
                                    <thead>
                                        <tr> 
                                            ...
                                            <th class="visible-xs visible-sm visible-md visible-lg" style="width:220px;">Name</th>
                                            <th class="visible-md visible-lg" style="width:220px;">Group Name</th>
                                            <th class=" visible-sm visible-md visible-lg" style="width:120px;">Arrival Date</th>
                                            <th class="visible-sm visible-md visible-lg" style="width:120px;">Departure Date</th>
                                            <th class="visible-md visible-lg" style="width:100px;">No. of Days</th>
                                            <th class="visible-xs visible-sm visible-md visible-lg">Details</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        //$rs = mysql_query("SELECT gc.grp_id, gr.GroupName, gr.GroupArrivalDate, gr.GroupDepartureDate, gr.Pms_Package_ID, p.Package_Name, p.Arrival_Start_Date, p.Arrival_End_Date, p.Package_Max_Days FROM group_contacts AS gc INNER JOIN groups AS gr ON gr.grp_id = gc.grp_id INNER JOIN packages AS p ON p.Pms_Package_ID=gr.Pms_Package_ID WHERE gc.ContactID='".$_REQUEST['contactid']."' AND gr.GroupArrivalDate <> '0000-00-00' ORDER BY gr.GroupArrivalDate ASC");
                                        $rs = mysql_query("SELECT gc.grp_id, gc.ContactID, gr.Contact_ID, gr.GroupName, gr.GroupArrivalDate, gr.GroupDepartureDate, gr.Pms_Package_ID, gr.Booking_Status, gr.grp_total_cust, p.Package_Name, p.Package_Max_Days, c.ContactFirstName, c.ContactLastName, c.is_email FROM group_contacts AS gc INNER JOIN groups AS gr ON gr.grp_id = gc.grp_id AND gr.GroupArrivalDate<>'0000-00-00' INNER JOIN packages AS p ON p.Pms_Package_ID=gr.Pms_Package_ID LEFT OUTER JOIN contacts AS c ON c.ContactID=gc.ContactID WHERE gc.ContactID='" . $_REQUEST['contactid'] . "' ORDER BY gc.grp_id");
                                        if (mysql_num_rows($rs) > 0) {
                                            while ($row = mysql_fetch_object($rs)) {
                                                ?>
                                                <tr> 
                                                                        <!--<td class="visible-xs visible-sm visible-md visible-lg"><a href="<?php //print($_SERVER['PHP_SELF'] . "?show=1&cont_id=".$_REQUEST['cont_id']."&contactid=".$_REQUEST['contactid']."&grp_id=".$row->grp_id);  ?>"><?php //print($row->Package_Name);  ?></a></td>-->
                                                    <td class="visible-xs visible-sm visible-md visible-lg" style="text-transform:capitalize;"><a href="<?php print($_SERVER['PHP_SELF'] . "?show=1&cont_id=" . $_REQUEST['cont_id'] . "&contactid=" . $_REQUEST['contactid'] . "&grp_id=" . $row->grp_id); ?>"><?php print($row->ContactFirstName . " " . $row->ContactLastName); ?></a></td>
                                                    <td class="visible-md visible-lg"><?php print($row->GroupName); ?></td>
                                                    <td class="visible-sm visible-md visible-lg"><?php echo calendarDateConver2($row->GroupArrivalDate); //print($row->GroupArrivalDate);  ?></td>
                                                    <td class="visible-sm visible-md visible-lg"><?php echo calendarDateConver2($row->GroupDepartureDate); //print($row->GroupDepartureDate);   ?></td>
                                                    <td class="visible-md visible-lg"><?php print($row->Package_Max_Days); ?></td>
                                                    <td class="visible-xs visible-sm visible-md visible-lg"><div class="tooltips"><a href="<?php print($_SERVER['PHP_SELF'] . "?show=1&cont_id=" . $_REQUEST['cont_id'] . "&contactid=" . $_REQUEST['contactid'] . "&grp_id=" . $row->grp_id); ?>" data-original-title="Travel Info" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-plane"></i></a></div>
                <?php if ($_SESSION["UType"] == 1) { ?>
                                                            <div class="tooltips"><a href="<?php print("manage_rooms_assignment.php?show=1&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id . "&contactid=" . $row->ContactID); ?>" target="_blank" data-original-title="Room Assignments" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-home"></i></a></div>
                                                            <div class="tooltips"><a href="<?php print("manage_massage_assignment.php?show=1&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id . "&contactid=" . $row->ContactID); ?>" target="_blank" data-original-title="Massage Schedule" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-calendar"></i></a></div>
                                                            <div class="tooltips"><a href="<?php print("manage_advent_activity_assignment.php?show=1&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id . "&contactid=" . $row->ContactID); ?>" target="_blank" data-original-title="Adventure Activities" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-group"></i></a></div>
                                                            <div class="tooltips"><a href="<?php print("manage_boat_assignment.php?show=1&pms_pak_id=" . $row->Pms_Package_ID . "&grp_id=" . $row->grp_id . "&contactid=" . $row->ContactID); ?>" target="_blank" data-original-title="Boat Schedules" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-location-arrow"></i></a></div>
                                                            <div class="tooltips"><a href="<?php print("manage_activity_logs.php?grp_id=" . $row->grp_id . "&contactid=" . $row->ContactID); ?>" target="_blank" data-original-title="Activity Logs" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-tasks"></i></a></div>
                <?php } else { ?>
                                            ...
                                            print('<tr><td colspan="100%" align="center">No Package Details founds</td></tr>');
                                        }
                                        ?>
                                    </tbody>
                            ...
                            print("<h3> Travel Information </h3>");
                            //$ti = mysql_query("SELECT * FROM travel_info WHERE cont_id='".$_REQUEST['cont_id']."' AND ContactID='".$_REQUEST['contactid']."' AND grp_id <> 0");
                            //print("SELECT t.*, g.Pms_Package_ID, p.Package_Name FROM travel_info AS t LEFT OUTER JOIN groups AS g ON g.grp_id=t.grp_id LEFT OUTER JOIN packages AS p ON p.Pms_Package_ID=g.Pms_Package_ID WHERE t.cont_id='".$_REQUEST['cont_id']."' AND t.ContactID='".$_REQUEST['contactid']."' AND t.grp_id <> 0");
                            //$ti = mysql_query("SELECT t.*, g.Pms_Package_ID, p.Package_Name FROM travel_info AS t LEFT OUTER JOIN groups AS g ON g.grp_id=t.grp_id LEFT OUTER JOIN packages AS p ON p.Pms_Package_ID=g.Pms_Package_ID WHERE t.cont_id='".$_REQUEST['cont_id']."' AND t.ContactID='".$_REQUEST['contactid']."' AND t.grp_id <> 0");
                            $ti = mysql_query("SELECT t.*, g.Pms_Package_ID, p.Package_Name FROM travel_info AS t LEFT OUTER JOIN groups AS g ON g.grp_id=t.grp_id LEFT OUTER JOIN packages AS p ON p.Pms_Package_ID=g.Pms_Package_ID WHERE t.cont_id='" . $_REQUEST['cont_id'] . "' AND t.ContactID='" . $_REQUEST['contactid'] . "' AND t.grp_id=" . $_REQUEST['grp_id']);
                            if (mysql_num_rows($ti) > 0) {
                                $tir = mysql_fetch_object($ti);
                                $packageName = $tir->Package_Name;
                                $arr_flight_id = $tir->arr_flight_id;
                                $arr_flightn_id = $tir->arr_flightn_id;
                                $arr_flightt_id = $tir->arr_flightt_id;
                                $arr_hotel_id = $tir->arr_hotel_id;
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-10 col-md-9 det-display"><?php echo $packageName; ?></div>
                                </div>
                                <div class="form-group">
                                    <label for="site_login" class="col-lg-2 col-md-3 control-label">Arrival Mode</label>
            ...
            <?php } ?>
                                ...
                                <div class="form-group">
                                    <label for="site_login" class="col-lg-2 col-md-3 control-label">Arrival & Departure Commets</label>
                                    <div class="col-lg-10 col-md-9 det-display"><?php echo $con_flight_comments; ?></div>
                                </div>
            <?php
        } else {
            ?>
                                <div class="form-group">
                                    <label for="site_login" class="col-lg-2 col-md-3 control-label">&nbsp;</label>
                                    <div class="col-lg-10 col-md-9 det-display">Info Not Found</div>
                                </div>
                                <?php
                            }
                            ?>
                            ...
                            $Query = "SELECT q.*, cpd.question_id AS cpdquestion_id, cpd.istrue, cpd.cpd_answer FROM questions AS q, contact_profile_details AS cpd WHERE q.question_id=cpd.question_id AND cpd.cont_id=" . $_REQUEST['contactid'] . " AND grp_id=" . $_REQUEST['grp_id'] . " AND cpd.cpd_answer!='' AND q.status_id=1 AND q.question_id>2 ORDER BY q.question_id";
                            $rs = mysql_query($Query);
                            if (mysql_num_rows($rs) > 0) {
                                while ($row = mysql_fetch_object($rs)) {
                                    $counter_qn++;
            ...
            ?>
                            ...
                            $Query = "SELECT a.*, cp.conp_id, ca.act_id AS caact_id FROM activities AS a LEFT OUTER JOIN contact_profiles AS cp ON cp.cont_id=" . $_REQUEST['contactid'] . " AND cp.grp_id=" . $_REQUEST['grp_id'] . " LEFT OUTER JOIN contact_activities AS ca ON a.act_id=ca.act_id AND ca.conp_id=cp.conp_id WHERE ca.act_id!='' AND a.status_id=1 ORDER BY a.act_order ";
                            $rs = mysql_query($Query);
                            if (mysql_num_rows($rs) > 0) {
                                while ($row = mysql_fetch_object($rs)) {
                                    $counter++;
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            <div class="tooltips">
                                                <div class="list-group"> <span style="float: left;"> &nbsp;  &nbsp; Activity Info:</span> <a href="javascript:void(0);" data-original-title="<?php echo $row->act_details; ?>" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-info-circle"></i></a> &nbsp; &nbsp; <span style="float: left;"> &nbsp;  Price:</span> <a href="javascript:void(0);" data-original-title="<?php echo $row->act_price; ?>" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-usd"></i></a> &nbsp; &nbsp; <span style="float: left;"> &nbsp;  Website:</span> <a href="<?php echo $row->act_link; ?>" target="_blank" data-placement="top" class="btn bg-primary text-white list-group-item" style="width:44px; float: left;"><i class="fa fa-link"></i></a> &nbsp; </div>
                                            </div>
                                        </div>
                                    </div>
                <?php
    ...
    ?>
                        ...
                        <div class="form-group">
                            <label for="site_login" class="col-lg-2 col-md-3 control-label">Any Other Comments</label>
                            <div class="col-lg-10 col-md-9 det-display"> <?php print($conp_comments); ?> 
...
//                    echo '<br/>';
        ...
        <div class="col-md-12">
            <div class="panel panel-cascade">
                <div class="panel-heading text-primary">
                                <?php //if($_SESSION["UType"]==1){  ?>
                    <h3 class="panel-title"> <i class="fa fa-user"></i> Guest Profile <span class="pull-right" style="width:auto;">
                            <div style="float:right;">
                                <?php if ($_SESSION["UType"] == 1) { ?>
                                            <!--  <a href="<?php print("sync/contacts.php?getContacts=1"); ?>"><i class="fa fa-plus"></i> Synchronize Contacts </a> | -->
                                <?php } ?>
                                <?php if ($_SESSION['group_id'] > 0) { ?>
        <?php if (($total_prof != $consumed_prof) && ($_SESSION['UType'] != 3)) { ?>
                                        <a href="<?php print($_SERVER['PHP_SELF'] . "?action=1&grp_id=" . $_SESSION['group_id']); ?>" title="Add New"><i class="fa fa-plus"></i> Add New </a> | <a href="<?php print($_SERVER['PHP_SELF'] . "?action=1&invite_guest=1&grp_id=" . $_SESSION['group_id']); ?>" title="Invite Guest"><i class="fa fa-plus"></i> Invite Guest </a> | <a href="<?php print($_SERVER['PHP_SELF'] . "?action=1&invite_guest=2&grp_id=" . $_SESSION['group_id']); ?>" title="Invite Guest"><i class="fa fa-plus"></i> Invite from past groups </a>
                        <?php } ?>
    <?php } ?>
                            </div>
                        </span> </h3>
    ...
    if ($_SESSION["UType"] > 1) {
        if ($_SESSION["UType"] == 2) {
            ?>
                                    <form name="frm210" id="frm210" method="post" class="form-horizontal cascde-forms" action="<?php print($_SERVER['PHP_SELF']); ?>">
                                        <div class="form-group">
                                            <label class="col-lg-2 col-md-3 control-label">Select Group:</label>
                                            <div class="col-lg-10 col-md-9">
                                                <select name="grp_id" id="grp_id" data-placeholder="Choose a Group..." class="chosen-select" style="width:240px; z-index: 9999 !important;" tabindex="2"  onChange="javascript:document.getElementById('frm210').submit()">
            <?php //echo FillSelected3(" groups AS g LEFT OUTER JOIN packages AS p ON g.Pms_Package_ID=p.Pms_Package_ID LEFT OUTER JOIN contacts AS c ON g.Contact_ID=c.ContactID WHERE g.GroupArrivalDate!='0000-00-00' AND g.GroupDepartureDate!='0000-00-00' AND g.Booking_Status='Confirmed' AND c.ContactID=".$_SESSION["contact_id"]." ", " g.grp_id ", " g.GroupName ", " g.GroupArrivalDate ", " p.Package_Name ", @$_SESSION['group_id']);  ?>
                                    <?php echo FillSelected3(" group_contacts AS gc INNER JOIN groups AS gr ON gr.grp_id = gc.grp_id INNER JOIN packages AS p ON p.Pms_Package_ID=gr.Pms_Package_ID WHERE gr.GroupArrivalDate!='0000-00-00' AND gr.GroupArrivalDate>'".date("Y-m-d",strtotime("-1 week"))."' AND gr.GroupDepartureDate!='0000-00-00' AND gr.Booking_Status='Confirmed' AND gc.ContactID='" . $_SESSION["contact_id"] . "'", " gc.grp_id ", " gr.GroupName ", " gr.GroupArrivalDate ", " p.Package_Name ", @$_SESSION['group_id']); ?>
                                                </select>
                                            </div>
                                        </div>
                                    </form>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-10 col-md-9">
                                            <select name="grp_id" id="grp_id" data-placeholder="Choose a Group..." class="chosen-select" style="width:240px; z-index: 9999 !important;" tabindex="2"  onChange="javascript:document.getElementById('frm22').submit()">
                                                <option value="0"> Show All </option>
                                <?php echo FillSelected3(" groups AS g LEFT OUTER JOIN packages AS p ON g.Pms_Package_ID=p.Pms_Package_ID LEFT OUTER JOIN contacts AS c ON g.Contact_ID=c.ContactID WHERE g.GroupArrivalDate!='0000-00-00' AND g.GroupDepartureDate!='0000-00-00' AND g.GroupArrivalDate>'".date("Y-m-d",strtotime("-1 week"))."'  AND g.Booking_Status='Confirmed' ", " g.grp_id ", " g.GroupName ", " g.GroupArrivalDate ", " p.Package_Name ", @$_SESSION['group_id']); ?>
                                            </select>
                                        </div>
                                    </div>
                                </form>
...
//    }
                                ...
                                if ($_SESSION["UType"] == 3) {
                                    ...
                                    if ($isLeader > 0) {
                                        //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE ( gc.grp_id = c.grp_id OR gc.ContactID = c.ContactID )  AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                        $Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE c.ContactID = gc.ContactID AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                    } else {
                                        $Query = "SELECT c.* FROM contacts as c WHERE c.ContactID=" . $_SESSION["contact_id"] . " ";
                                    }
                                } else {
                                    if ($_SESSION['group_id'] > 0) {
                                        //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE gc.grp_id = c.grp_id AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                        //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE ( gc.grp_id = c.grp_id OR gc.ContactID = c.ContactID )  AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                        //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE c.ContactID = gc.ContactID AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                        //$Query = "SELECT gc.*, c.*, t.arrival_flight_data AS arrival_date, t.departure_flight_date AS departure_date FROM group_contacts AS gc LEFT OUTER JOIN contacts AS c ON c.ContactID = gc.ContactID LEFT OUTER JOIN travel_info AS t ON t.ContactID=gc.ContactID AND t.grp_id=gc.grp_id WHERE gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY c.cont_id ";
                                        $Query = "SELECT gc.*, c.*, t.arrival_flight_data AS arrival_date, t.departure_flight_date AS departure_date FROM group_contacts AS gc LEFT OUTER JOIN contacts AS c ON c.ContactID = gc.ContactID LEFT OUTER JOIN travel_info AS t ON t.ContactID=gc.ContactID AND t.grp_id=gc.grp_id WHERE gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed>=1 GROUP BY gc.ContactID";
                                    } else {
                                ...
                                <!-- <td><input type="checkbox" name="chkstatus[]" value="<?php print($row->cont_id); ?>" /></td>-->
                                            <td class="visible-xs visible-sm visible-md visible-lg" style="text-transform:capitalize;"><?php print($row->cont_fname); ?></td>
                                            <td class="visible-xs visible-sm visible-md visible-lg" style="text-transform:capitalize;"><?php print($row->cont_lname); ?></td>
                                            ...
                                            <td style="width:180px"><?php if ($isLeader > 0) { ?>
                <?php //if($_SESSION["UType"]==1){   ?>
                                                    ...
                                                    <div class="tooltips"><a href="<?php print($_SERVER['PHP_SELF'] . "?action=2&cont_id=" . $row->cont_id . "&contactid=" . $row->ContactID); ?>&grp_id=<?php echo @$_SESSION['group_id']; ?>" data-original-title="Update Record" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-edit"></i></a></div>
                                                <?php } else { ?>
                                                    <div class="tooltips"><a href="<?php print($_SERVER['PHP_SELF'] . "?show=1&cont_id=" . $row->cont_id . "&contactid=" . $row->ContactID); ?>" data-original-title="See Details" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-eye"></i></a></div>
                                            <?php if ($_SESSION["contact_id"] == $row->ContactID) { ?>
                                                        <div class="tooltips"><a href="<?php print($_SERVER['PHP_SELF'] . "?action=2&cont_id=" . $row->cont_id . "&contactid=" . $row->ContactID); ?>&grp_id=<?php echo @$_SESSION['group_id']; ?>" data-original-title="Update Record" data-placement="top" class="btn bg-primary text-white" style="float:left; margin-left: 2px; margin-bottom: 2px;"><i class="fa fa-edit"></i></a></div>
                                            <?php } ?>
                                        <?php } ?></td>
                                        </tr>
...
//    }
                                    ...
                                    if ($_SESSION["UType"] == 3) {
                                        if ($isLeader > 0) {
                                            //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE ( gc.grp_id = c.grp_id OR gc.ContactID = c.ContactID )  AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed<=0 GROUP BY c.cont_id ";
                                            $Query = "SELECT gc.*, c.*, t.arrival_flight_data AS arrival_date, t.departure_flight_date AS departure_date FROM group_contacts AS gc LEFT OUTER JOIN contacts AS c ON c.ContactID = gc.ContactID LEFT OUTER JOIN travel_info AS t ON t.ContactID=gc.ContactID AND t.grp_id=gc.grp_id WHERE gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed<=0 GROUP BY c.cont_id ";
                                        } else {
                                            $Query = "SELECT c.* FROM contacts as c WHERE c.ContactID=" . $_SESSION["contact_id"] . " AND c.is_completed<=0 ";
                                        }
                                    } else {
                                        if ($_SESSION['group_id'] > 0) {
                                            // $Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE gc.grp_id=c.grp_id AND gc.grp_id=" . $_SESSION['group_id'] . "  AND c.is_completed<=0  GROUP BY c.cont_id ";
                                            //$Query = "SELECT gc.*, c.* FROM group_contacts AS gc, contacts AS c WHERE ( gc.grp_id = c.grp_id OR gc.ContactID = c.ContactID )  AND gc.grp_id=" . $_SESSION['group_id'] . " AND c.is_completed<=0 GROUP BY c.cont_id ";
                                            $Query = "SELECT gc.*, c.* FROM group_contacts AS gc LEFT OUTER JOIN contacts AS c ON c.ContactID=gc.ContactID WHERE gc.grp_id='" . $_SESSION['group_id'] . "' AND c.is_completed<=0 GROUP BY gc.ContactID";
                                        } else {
...
<script>
                                        /* Fish Processing Interaction */
                                        
                                        function ProcessTheFish() {
                                            
                                            if($('input[name=fish_smoked]:checked').val() == "1") {
                                                
                                                $('#row_fish_smoking_instructions').fadeIn();
                                                
                                            } else {
                                                
                                                $('#row_fish_smoking_instructions').fadeOut();
                                                
                                            }
                                            
                                            if($('#fish_delivery_type_id').val() == "2") {
                                                
                                                $('#row_shipping_instructions').fadeIn();
                                                
                                            } else {
                                                
                                                $('#row_shipping_instructions').fadeOut();
                                        ...
                                        $(document).ready(function(){
                                            ...
                                            $('.fish_smoked').on('change', function() {
                                                ProcessTheFish();
                                            });
                                            
                                            $('#fish_delivery_type_id').on('change', function() {
                                                ProcessTheFish();
                                            });
