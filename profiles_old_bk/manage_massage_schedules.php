...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Massage Schedules <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b>Manage Massage Schedules: </b> You can manage your Massage schedules here </p>
        </blockquote>
    </div>
</div>
        ...
        <div class="col-md-12">
            <div class="panel panel-cascade">
                <div class="panel-heading text-primary">
                    <h3 class="panel-title"><i class="fa fa-check"></i> Massage Schedule</h3>
                </div>
                <div class="panel-body ">
                    <div class="ro">
...
//                                                                            $counter4++;
                                                                ...
                                                                $counter3 = 0;
                                                                $Query3 = "SELECT cp.conp_id, cp.cont_id, ca.act_id, a.act_name, g.grp_id, g.GroupName, c.cont_fname, c.cont_lname, c.ContactID, asch.asch_id, asch.cont_id AS aschcont_id, asch.asch_start_date, asch.asch_end_date, asch.asch_duration, asch.asch_email_counter, ag.ag_name, ad.ad_name, av.av_name, abn.act_boat_name, thr.ath_id, thr.ath_name, thrt.atht_id, thrt.atht_name FROM  contact_profiles AS cp LEFT OUTER JOIN contact_activities AS ca ON cp.conp_id=ca.conp_id LEFT OUTER JOIN activities AS a ON ca.act_id=a.act_id LEFT OUTER JOIN act_schedule AS asch ON asch.grp_id=" . $row1->grp_id . " AND asch.cont_id=" . $row2->ContactID . " AND asch.act_id=a.act_id LEFT OUTER JOIN groups AS g ON g.grp_id=" . $row1->grp_id . " LEFT OUTER JOIN contacts AS c ON c.ContactID=" . $row2->ContactID . " LEFT OUTER JOIN activity_guides AS ag ON asch.ag_id=ag.ag_id LEFT OUTER JOIN activity_destination AS ad ON asch.ad_id=ad.ad_id LEFT OUTER JOIN activity_vendors AS av ON asch.av_id=av.av_id LEFT OUTER JOIN act_therapist AS thr ON asch.ath_id=thr.ath_id LEFT OUTER JOIN act_th_type AS thrt ON asch.atht_id=thrt.atht_id LEFT OUTER JOIN act_boats AS abn ON asch.act_boat_id=abn.act_boat_id WHERE  
#( cp.cont_id=" . $row2->ContactID . " OR  asch.cont_id=" . $row2->ContactID . " ) AND 
a.act_id !='' AND a.act_id=1 GROUP BY a.act_id ORDER BY a.act_order ASC";
                                                                $count3 = mysql_num_rows(mysql_query($Query3));
                                                                $rs3 = mysql_query($Query3);
                                                                if (mysql_num_rows($rs3) > 0) {
                                                                    ?>
                                                                                        ...
                                                                                        if ($no_of_days == $day_counter) {
                                                                                            ...
                                                                                            if ($no_of_days != (1 + $day_counter)) {
                                                                                                ...
                                                                                                if ($row3->asch_id != 0 || $row3->asch_id != '') {
                                                                                                    ...
                                                                                                    $counter5 = 0;
                                                                                                    $Query5 = "SELECT asch.asch_id FROM act_schedule AS asch WHERE asch.act_id=".$row3->act_id." AND asch.cont_id = ".$row3->aschcont_id." AND asch.grp_id = ".$row3->grp_id." AND asch.asch_start_date LIKE '".$dt->format("Y-m-d")."%' AND asch.act_id = 1 LIMIT 1";
...
//                                                                                                            - <div class='tooltips'><a href=".$_SERVER['PHP_SELF'] . "?send=1&cont_id=" . $row3->aschcont_id ."&act_id=" . $row3->act_id."&asch_id=" . $row3->asch_id." data-original-title= 'Email has been sent ".$row3->asch_email_counter.(($row3->asch_email_counter>1)?" times ":" time ")."' data-placement='top' class='btn bg-primary text-white' style='float:left; margin-left: 2px; margin-bottom: 2px;'><i class='fa fa-envelope-o'></i></a></div>";
                                                                                                        ...
                                                                                                        //echo $dt->format("m/d/Y");
                                                                                                        echo "<a href=" . $_SERVER['PHP_SELF'] . "?action=1&sch=0&cont_id=".$row3->ContactID. "&grp_id=".$row3->grp_id."&act_id=".$row3->act_id."&asch_id=&pms_pak_id=".$row1->Pms_Package_ID."&date=".$dt->format("Y-m-d").">Not Scheduled1</a>";
                                                                                                    }
                                                                                                } else {
                                                                                                    echo "<a href=" . $_SERVER['PHP_SELF'] . "?action=1&sch=0&cont_id=".$row3->ContactID."&grp_id=".$row3->grp_id."&act_id=".$row3->act_id."&asch_id=&pms_pak_id=".$row1->Pms_Package_ID."&date=".$dt->format("Y-m-d").">Not Scheduled2</a>";
                                                                                                }
                                                                                                echo '</th>';
                                                                                            }
                                                                                        }
