...
if (isset($_REQUEST['btnAdd'])) {
    ...
    for($i=0; $i<count($_REQUEST['ftype_id']); $i++){
        if(isset($_REQUEST['frec_id']) && $_REQUEST['frec_id']!=''){
            
//            echo "UPDATE fish_record SET ftype_id = '" . dbStr($_REQUEST['ftype_id'][$i]) . "', frec_count = '" . dbStr($_REQUEST['frec_count'][$i]) . "', frec_weight = '" . dbStr($_REQUEST['frec_weight'][$i]) . "', frec_recovery = '" . dbStr($_REQUEST['frec_recovery'][$i]) . "', frec_filets_weight = '" . dbStr($_REQUEST['frec_filets_weight'][$i]) . "', lastUpdated=NOW() WHERE cont_id = ".$_REQUEST['cont_id']." AND  frec_date = '" . dbStr($_REQUEST['act_date']) . "' ";
//            echo '<br/>';
//            
//            mysql_query("UPDATE fish_record SET ftype_id = '" . dbStr($_REQUEST['ftype_id'][$i]) . "', frec_count = '" . dbStr($_REQUEST['frec_count'][$i]) . "', frec_weight = '" . dbStr($_REQUEST['frec_weight'][$i]) . "', frec_recovery = '" . dbStr($_REQUEST['frec_recovery'][$i]) . "', frec_filets_weight = '" . dbStr($_REQUEST['frec_filets_weight'][$i]) . "', lastUpdated=NOW() WHERE cont_id = ".$_REQUEST['cont_id']." AND  frec_date = '" . dbStr($_REQUEST['act_date']) . "' ");
        } else {
            $maxID = getMaximum("fish_record", "frec_id");
            mysql_query("INSERT INTO fish_record (frec_id, grp_id, cont_id, pms_pak_id, ftype_id, frec_count, frec_weight, frec_recovery, frec_filets_weight, frec_date, createdDate) VALUES( '" . $maxID . "', '" . dbStr($_REQUEST['grp_id']) . "', '" . dbStr($_REQUEST['cont_id']) . "', '" . dbStr($_REQUEST['pms_pak_id']) . "', '" . dbStr($_REQUEST['ftype_id'][$i]) . "', '" . dbStr($_REQUEST['frec_count'][$i]) . "', '" . dbStr($_REQUEST['frec_weight'][$i]) . "', '" . dbStr($_REQUEST['frec_recovery'][$i]) . "', '" . dbStr($_REQUEST['frec_filets_weight'][$i]) . "', '" . dbStr($_REQUEST['act_date']) . "' , NOW())");
        }
    }
    
    //die();
    header("Location: manage_fish_processing.php?show=1&pms_pak_id=" . $_REQUEST['pms_pak_id'] . "&op=1");
} 
                                ...
                                <form class="form-horizontal cascde-forms" method="post" action="<?php print($_SERVER['PHP_SELF']); ?>" name="frm" id="frm" >
                                    ...
                                    <table class="table users-table table-condensed table-hover table-striped display dataTable" id="" >
                                        ...
                                        <tr>
                                            ...
                                            <td class="visible-xs visible-sm visible-md visible-lg">
                                                <input type="text" class="form-control form-cascade-control input_wid70" value="<?php echo @$row->frec_count;?>" id="frec_count[]" name="frec_count[]">
                                            </td>
                                            <td class="visible-xs visible-sm visible-md visible-lg">
                                                <input type="text" class="form-control form-cascade-control input_wid70" value="<?php echo @$row->frec_weight;?>" id="frec_weight[]" name="frec_weight[]">
                                            </td>
                                            <td class="visible-xs visible-sm visible-md visible-lg">
                                                <input type="text" class="form-control form-cascade-control input_wid70" value="<?php echo @$row->frec_recovery;?>" id="frec_recovery[]" name="frec_recovery[]">
                                            </td>
                                            <td class="visible-xs visible-sm visible-md visible-lg">
                                                <input type="text" class="form-control form-cascade-control input_wid70" value="<?php echo @$row->frec_filets_weight;?>" id="frec_filets_weight[]" name="frec_filets_weight[]">
                                            </td>
                                        </tr>
                                        <?php
                                                }
                                            ...
                                            <tr>
                                                <td class="visible-xs visible-sm visible-md visible-lg">
                                                    ...
                                                    <?php
                                                        ...
                                                        LEFT OUTER JOIN contact_profiles AS cp ON c.ContactID=cp.cont_id
                                                        LEFT OUTER JOIN jacket_size AS j ON cp.jacketsize_id=j.jacketsize_id
                                                        WHERE p.Pms_Package_ID = ".$row->Pms_Package_ID." AND p.Arrival_Start_Date > '2014-05-01' AND g.grp_id=c.grp_id
                                                        GROUP BY g.grp_id 
                                                        ...
                                                        if ($count11 > 0) {
                                                            while ($row11 = mysql_fetch_object($rs11)) {
                                                                echo "<a href=".$_SERVER['PHP_SELF'] . "?show=1&pms_pak_id=".$row->Pms_Package_ID."&grp_id=".$row11->grp_id."><br/>".$row11->GroupName."</a>";
                                                            }
                                                        }
                                                    ?>
                                                </td>
