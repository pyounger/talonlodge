                ...
                <div class="panel-body">
                    <form name="frm" id="frm" method="post" action="<?php print($_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING']); ?>" class="form-horizontal" role="form">
                        <table class="table users-table table-condensed table-hover table-striped display dataTable" id="example" >
                            ...
                            <tbody>
                                <?php
                                    $Query = "SELECT u.*, c.ContactFirstName, c.ContactLastName FROM users AS u LEFT OUTER JOIN contacts AS c ON u.cont_id=c.ContactID WHERE user_name LIKE '".$_SESSION['search_name']."%' OR user_display_name LIKE '".$_SESSION['search_name']."%'  ORDER BY u.user_id DESC ";
                                    $counter = 0;
