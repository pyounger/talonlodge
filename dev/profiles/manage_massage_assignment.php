    ...
    for ($i = 0; $i < count($_REQUEST['ath_id']); $i++) {
        if($_REQUEST['ath_id'][$i] != 0){
            if($_REQUEST['asch_id'][$i] != 0){
                ...
                mysql_query("UPDATE act_schedule SET ath_id = '" . $_REQUEST['ath_id'][$i] . "', atht_id = '" . $_REQUEST['atht_id'][$i] . "', asch_start_date = '" . $asch_start_date . "', asch_end_date = '" . $asch_end_date . "', asch_duration = '" . $_REQUEST['asch_duration'][$i] . "', updatedDate = NOW() WHERE asch_id = ".$_REQUEST['asch_id'][$i]." ") or die(mysql_error());
                ...
                mysql_query("INSERT INTO act_schedule (asch_id, cont_id, grp_id, act_id, ath_id, atht_id, asch_start_date, asch_end_date, asch_duration, createdDate, pms_pak_id) VALUES (".$MaxID.", '".$_REQUEST['cont_id'][$i]."', '".$_REQUEST['grp_id'][$i]."', '1', '".$_REQUEST['ath_id'][$i]."', '".$_REQUEST['atht_id'][$i]."', '".$asch_start_date."', '".$asch_end_date."', '".$_REQUEST['asch_duration'][$i]."', NOW(), '".$_REQUEST['pms_pak_id']."')");
