...
<?php } else { ?>
    ...
    ?>
	<?php if((isset($_REQUEST['grp_id'])) && (isset($_REQUEST['contactid']))){ ?>
	<?php } else{ ?>
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-cascade">
                            ...
                            <tbody>
                                <?php
                                    /*$Query = "
										...
										$filterQry = "";
										if((isset($_REQUEST['grp_id'])) && (isset($_REQUEST['contactid']))){
											$filterQry = " WHERE asch.grp_id=".$_REQUEST['grp_id']." AND asch.cont_id=".$_REQUEST['contactid'];
										}
										else{
											$filterQry = " WHERE asch.asch_start_date LIKE '".$date2."%'";
										}
									$Query = "
                                        SELECT 
                                        asch.*, al.actl_id, al.actl_details, ac.act_name, c.ContactFirstName, c.ContactLastName, g.GroupName, actt.ath_name, actht.atht_name, ab.*, bc.actboca_name, bcd.actbodh_name, ag.ag_name, av.av_name
                                        FROM 
                                        act_schedule AS asch 
                                        LEFT OUTER JOIN activity_logs AS al ON al.asch_id = asch.asch_id
                                        LEFT OUTER JOIN activities AS ac ON ac.act_id = asch.act_id
                                        LEFT OUTER JOIN contacts AS c ON c.ContactID = asch.cont_id
                                        LEFT OUTER JOIN groups AS g ON g.grp_id = asch.grp_id
                                        LEFT OUTER JOIN act_therapist AS actt ON actt.ath_id=asch.ath_id
                                        LEFT OUTER JOIN act_th_type AS actht ON actht.atht_id=asch.atht_id
                                        LEFT OUTER JOIN act_boats AS ab ON ab.act_boat_id=asch.act_boat_id 
                                        LEFT OUTER JOIN act_boat_captain AS bc ON bc.actboca_id = ab.actboca_id
                                        LEFT OUTER JOIN act_boat_deckhand AS bcd ON bcd.actbodh_id = ab.actbodh_id
                                        LEFT OUTER JOIN activity_guides AS ag ON ag.ag_id=asch.ag_id
                                        LEFT OUTER JOIN activity_vendors AS av ON av.av_id=asch.av_id
                                        ".$filterQry."
                                        ORDER BY g.GroupName, c.ContactFirstName ASC";
                                    $counter = 0;
