...
if(isset($_REQUEST['submitAllRec'])){
    for ($i = 0; $i < count($_REQUEST['act_id']); $i++) {
        ...
        $arr2 = explode(',',$_REQUEST['av_id'][$i]);
        $arr3 = explode(',',$_REQUEST['ag_id'][$i]);
        ...
        if($arr1[0] != 0){
            ...
            if($asch_id != 0){
                mysql_query("UPDATE act_schedule SET act_id = '" . $act_id . "', ag_id = '" . $ag_id . "', av_id = '" . $av_id . "', updatedDate = NOW() WHERE asch_id = ".$asch_id." ") or die(mysql_error());
                ...
                mysql_query("INSERT INTO act_schedule (asch_id, cont_id, grp_id, act_id, asch_start_date, asch_end_date, ag_id, av_id, createdDate, pms_pak_id) VALUES (".$MaxID.", '".$cont_id."', '".$grp_id."', '".$act_id."', '".$asch_start_date."', '".$asch_start_date."', '".$ag_id."', '".$av_id."', NOW(), '".$_REQUEST['pms_pak_id']."')");
...
<div class="row">
    <div class="col-mod-12">
        ...
        <h3 class="page-header"> Manage Adventure Activity  Assignment <i class="fa fa-info-circle animated bounceInDown show-info"></i></h3>
        <blockquote class="page-information hidden">
            <p> <b> Manage Adventure Activity  Assignment </b> You can Manage Adventure Activity  Assignment here </p>
        </blockquote>
    </div>
</div>
...
//                                                    echo '</th>';
                                                        ...
                                                        if ($day_counter != 1) {
                                                            if ($no_of_days == $day_counter) {
                                                                ...
                                                                if ($no_of_days != (1 + $day_counter)) {
                                                                    ...
                                                                    echo $Query99 = "
                                                                    SELECT 
                                                                    asch.asch_id, asch.ag_id, asch.av_id, asch.act_id, asch.asch_start_date, acn.act_name
                                                                    FROM  
                                                                    act_schedule AS asch
                                                                    LEFT OUTER JOIN activities AS acn ON asch.act_id = acn.act_id
                                                                    WHERE 
                                                                    asch.cont_id = ".$row->ContactID." AND asch.act_id NOT IN (1,2,7,11) 
                                                                    ...
                                                                    ?>
                                                                        ...
                                                                        <select name="act_id[]" data-placeholder="Choose an Activity..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                            <?php
                                                                                $strQueryv = "SELECT act_id, act_name FROM activities WHERE act_id NOT IN (1,2,7,11) ORDER BY act_name";
                                                                        ...
                                                                        <br/><br/>
                                                                        VENDOR: <br/> 
                                                                        <select name="av_id[]" id="av_id[]" data-placeholder="Choose a Vendor..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                            <?php
                                                                                $strQueryv = "SELECT av_id, av_name FROM activity_vendors ORDER BY av_name ";
                                                                                $nResultv = mysql_query($strQueryv);
                                                                                if (mysql_num_rows($nResultv) >= 1) {
                                                                                    while ($rowv = mysql_fetch_row($nResultv)) {
                                                                                        if ($rowv[0] == $row99->av_id) {
                                                                                            echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->ContactID.','.$row->grp_id.','.$row99->asch_id." selected>".$rowv[1]."</option>";
                                                                                        } else {
                                                                                            echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->ContactID.','.$row->grp_id.','.$row99->asch_id.">".$rowv[1]."</option>";
                                                                                        }
                                                                        ...
                                                                        <br/><br/>
                                                                        GUIDE: <br/> 
                                                                        <select name="ag_id[]" id="ag_id[]" data-placeholder="Choose a Guide..." class="chosen-" style="width:170px; z-index: 9999 !important;" tabindex="2">
                                                                            <?php
                                                                                $strQueryv = "SELECT ag_id, ag_name FROM activity_guides ORDER BY ag_name ";
                                                                                $nResultv = mysql_query($strQueryv);
                                                                                if (mysql_num_rows($nResultv) >= 1) {
                                                                                    while ($rowv = mysql_fetch_row($nResultv)) {
                                                                                        if ($rowv[0] == $row99->ag_id) {
                                                                                            echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->ContactID.','.$row->grp_id.','.$row99->asch_id." selected>".$rowv[1]."</option>";
                                                                                        } else {
                                                                                            echo "<option value=".$rowv[0].','.$dt->format("Y-m-d").','.$row->ContactID.','.$row->grp_id.','.$row99->asch_id.">".$rowv[1]."</option>";
                                                                                        }
                                                            ...
                                                            $Query99 = "
                                                                    SELECT 
                                                                    asch.asch_id, asch.ag_id, asch.av_id, asch.act_id, asch.act_boat_id, asch.asch_start_date, acn.act_name, actb.act_boat_name
                                                                    FROM  
                                                                    act_schedule AS asch
                                                                    LEFT OUTER JOIN activities AS acn ON asch.act_id = acn.act_id
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel">
                    <div class="panel-heading text-primary">
                        <h3 class="panel-title"><i class="fa fa-glass"></i> Manage Adventure Activity  Assignment 
                            <span class="pull-right" style="width:auto;">
                            </span> 
                        </h3>
                    </div>
