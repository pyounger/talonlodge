...
{block name='title'}
    ...
    {capture name='site_title'}{t}Testings{/t}{/capture}
...
{block name='content_init'}
    ...
    {if $cpf_rights->has_rights($cpf_controller, 'add')}
        ...
        {capture name="add_url"}{link controller=$cpf_controller action='add' ctype=$type}?r={$cpf_url_current}{/capture}

        {capture name="common_actions"}

            {cpf_button title=t('Add Category') icon='plus-sign' url=$smarty.capture.add_url}
...
{block name='content_filter'}
	...
	{include file='backend_testings.submenu.tpl'}
