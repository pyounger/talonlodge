...
{block name='content_init'}
    ...
    {if $cpf_rights->has_rights($cpf_controller, 'add')}
        ...
        {capture name="common_actions"}
            ...
            {cpf_button title=t('Add Testing') icon='plus-sign' url=cpf_link(['controller' => $cpf_controller, 'action' => 'add'])}
...
{block name='content'}
    ...
    <tr class="{cycle values=',alternate'}">
        ...
        <td>
            ...
            {if $cpf_rights->has_rights('backend_testings', 'edit')}

                <a href="{link controller='backend_testings' action="edit" id=$item->id}">{$item->title}</a>
