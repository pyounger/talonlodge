...
{block name='title'}
    {capture name='site_title'}{t}backend.navigation.menu{/t}{/capture}
...
{block name='content_init'}
    ...
    {if $cpf_rights->has_rights($cpf_controller, 'add')}
        {capture name="common_actions"}
            {cpf_button title=t('backend.navigation.add_menu') icon='plus-sign' url=cpf_link(['controller' => $cpf_controller, 'action' => 'add'])}
        {/capture}
    {/if}
...
{block name='content'}
    ...
    <tr class="{cycle values=',alternate'}">
        ...
        {* title *}
        <td>
            {if $cpf_rights->has_rights('backend_navigationelements', 'view')}
                <a href="{link controller='backend_navigationelements' action="view" id=$item->id}">{$item->title}</a>
            {else}
                {$item->title}
            {/if}
        </td>
        ...
        {if $show_actions}
            <td>
                {if $cpf_rights->has_rights('backend_navigationelements', 'view')}
                    <a href="{link controller='backend_navigationelements' action='view' id=$item->id}"><i class="icon-list"></i> {t}backend.navigation.items{/t}</a>
                {/if}
