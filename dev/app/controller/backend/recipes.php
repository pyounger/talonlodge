<?php
class App_Controller_Backend_Recipes extends App_Controller_Base_BackendSmart
{
    public
        $model = 'App_Model_Recipe';

    public function __construct()
    {
        parent::__construct();
        $this->attachment_path = $this->config->value('APP.RECIPES.PATH');
        $this->attachment_url = $this->config->value('APP.RECIPES.URL');
    ...
    public function action_edit()
    {
		...
		{
			$categories = $this->outlet->from('App_Model_RecipesInCategory')->where('recipe_id = ?', array($entity->id))->find();
			$result = array();
    ...
    protected function upload_attachment($entity, $file)
    {
		...
		$w = $this->config->value('APP.RECIPES.FULL_WIDTH');
		$h = $this->config->value('APP.RECIPES.FULL_HEIGHT');
		...
		$w = $this->config->value('APP.RECIPES.WIDTH');
		$h = $this->config->value('APP.RECIPES.HEIGHT');
	...
	private function _check_categories($entity)
	{
		if ($this->request->is_post)
		{
			if ($entity->id > 0)
			{
				$this->outlet->query('delete from {App_Model_RecipesInCategory} WHERE recipe_id = ?', array($entity->id));
			}
			...
			foreach ($fields as $field)
			{
				if ($this->request->is_post && ($categories = $this->request->post($field)) !== FALSE && is_array($categories))
				{
					if ($entity->id > 0)
					{
						foreach ($categories as $category)
						{
							$c = new App_Model_RecipesInCategory();
							$c->category_id = $category;
							$c->recipe_id = $entity->id;
							$this->outlet->save($c);
						}
					}
