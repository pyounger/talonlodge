<?php
class App_Controller_Frontend_Reservations extends App_Controller_Base_Frontend
{
    public $errors = array();
    ...
    public function action_default()
    {
        ...
        {
            $minDate = $this->outlet->query('SELECT MIN(`Arrival_Start_Date`) FROM {App_Model_Reservation}')->fetchColumn();
        ...
        // load years
        $dates = $this->outlet->query('SELECT `Arrival_Start_Date`,`Arrival_End_Date` FROM {App_Model_Reservation}')->fetchAll(PDO::FETCH_ASSOC);
        foreach ($dates as $dd)
        {
        ...
        {
            $start = trim(sprintf('%d-01-01',$year));
        }
    ...
    public function action_update()
    {
        ...
        if (count($json) > 0)
        {
            $this->outlet->query('DELETE FROM {App_Model_Reservation}');
    ...
    public function action_view()
    {
        ...
        if (($slug = $this->request->param('slug')) !== FALSE && !is_null($package = $this->outlet->from('App_Model_Reservation')->where('slug=?', array($slug))->findOne()))
        {
     ...
     $add = isset($_SESSION['reservation_adults']) ? $_SESSION['reservation_adults'] : $package->Package_Min_Adults;
    ...
    if ($this->request->is_post)
    {
        ...
        if ($this->_validate())
        {
            ...
            if($package->Account_ID == 7){

                $directory = "Talon Lodge & Spa - Alaska Fishing Resort";

            }else if($package->Account_ID == 185){
            ...
            curl_close ($ch);
                    ...
                    */
                    ...
                    if ($result)
                    {
                        ...
                        $this->view = new Cpf_Core_View_Redirect(sprintf('%s?success=1', $this->router->link="reservations/{$package->slug}"));
       ...
       // echo "test2"; exit;
            $this->redirect('frontend_reservations');
        }
