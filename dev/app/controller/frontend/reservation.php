...
class App_Controller_Frontend_Reservation extends App_Controller_Base_Frontend
{
	...
	{
		$json = file_get_contents('http://www.magnusadventures.com/webservices/talonlodge/Packages_New.php');
		$json = sprintf('[%s]', $json);
	...
	public function action_view()
	{
		...
		//if ($_SERVER['HTTP_HOST'] == "http://www.dev.talonlodge.com") {
				
	    //$start = $_SESSION["start"];	
	   // $end = $_SESSION["end"];
	   // $sessadults = $_SESSION["adults"];	
	    $this->assign('start', $_SESSION["start"]);
	    $this->assign('end', $_SESSION["end"]);
	    $this->assign('sessadults', $_SESSION["adults"]);
	   // }
			...
			if ($this->request->is_post)
			{
				...
				if ($this->_validate())
				{
					...
					$post = array(
						...
						'Num_Adults' => isset($_SESSION['reservation_adults']) ? $_SESSION['reservation_adults'] : $PackageDetailByID[0]['Package_Min_Adults'],
